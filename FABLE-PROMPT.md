...
**Effort: run this session at `max` reasoning effort.** This is a reinvention where shallow decisions
are costly and quality is the whole point, so think deeply throughout. The single step to think
hardest about is **redesigning the backend without losing the club's live content or its ability to
edit it** — treat that as keep-or-deliberately-migrate, never lose. (`max` is the reasoning tier — not
the token-heavy multi-agent mode.)
