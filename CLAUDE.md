...
**And it multiplied them anyway, a different way.** A record saved by this form holds each goal's assist twice: on the goal, and in the flat list derived from it. Nothing took the second copy out when the record was opened again, it paired with nothing because the minutes were blank, and it was kept as an orphan and written back beside a freshly derived list. Every save added the goals' assists once more: the Three Little Birds match of 6 September 2026 reached **eight assists for two goals** in one evening of edits, and the site published four each for Charlie Dunkley and Jon Lloyd. A goal carrying its assist now takes its own entry out of the pool first (by `forGoalBy` where the entry has it), and **an assist has to belong to a goal**: orphans are kept, but never more of them than there are goals without an assist. Every stored match in the archive already satisfied that, which is what made it safe. The site applies the same rule where it reads stored matches (`reconcileAssists()` in `stats.mjs`), so an inflated record publishes correctly without anybody re-saving it, and the suite holds the two copies to agreeing on every stored match.
...
- **A formatted edition of a stored match report** (`src/data/report-editions.json`, applied in `dataset.mjs`): the club's report word for word with headings, tables, lists, graphics and profile links added, built by a script from the stored text and carrying `basedOn`, the sha256 of that raw text. It is the report the site shows only while the stored text still hashes to `basedOn`; an edit in the panel changes the hash and the stored text wins. `plainText` drops heading markers and table rows from previews. First used for the Haydons Park 8-1 (graphics `assets/articles/haydons-every-goal.jpg` and `haydons-player-of-the-match.jpg`).
- **Additions to a stored article** (`src/data/article-additions.json`, applied in `dataset.mjs` after the articles are assembled): a line the club wants in a piece that lives in the database, placed after the paragraph ending with `after` or at the end, and skipped once the stored text contains its `unless`. The stored text is never changed and the panel shows the text without the addition. First used for the Ade Owolana graphic and profile link. A `wrap` addition turns the first mention of some words into `[words](href)`, so a player's name can link to his profile. Links in an article body are orange and underlined (`.nw-art__body a`); before that rule a link looked exactly like the sentence around it. The homepage's latest cards wear the drawn covers (the plate for an article, the scoreline card for a report), so the plate's rules and Archivo live in `20-home.css`.
- **A link in an article is `[words](address)`**, a page on the site (`/players/ade-owolana.html`) or an https address only, read after escaping and never inside a picture line; `plainText` keeps the words. The panel's Link button wraps the selected words. A graphic made here for an article is committed under `assets/articles/` and placed with `!![caption](/assets/articles/<file>.jpg#WxH)`, kept under 100KB so it needs no srcset. The plate's headline and club name are **Archivo**, declared in `39-media.css`: the site's `--display` is Geist, so `var(--display)` rendered the plate in Geist while every drawn card is Archivo.
- **An article's default cover is drawn in markup on the site** (`coverPlate` in `news.mjs`, styled in `39-media.css` with container units): the eyebrow, the headline in Archivo 500 and the foot, the build card's proportions, so a headline or category changed in the panel shows on the next publish. Only a real photograph is an `<img>`; a panel-drawn `cover-news-*`/`cover-match-*` or the build's `a-<slug>.jpg` is never the on-site cover of an article (reports keep their drawn scoreline cards). Those images are still the shared link's `og:image`, the build's card preferred over a panel-drawn one, and the panel redraws its own `cover-news-*` on every save, in the site's fonts, laid out like `articleCover()`.
- **Latest from the club** (`recentPosts` in `home-layout.mjs`, `latestBand` in `home.mjs`): articles and match reports dated within seven days by the London calendar, newest first, straight after the League Eight bands whatever the saved order says; `10-home.js` ages cards out again in the browser and hides the band when the week empties. It carries no rail number and is not a `sec sec--` band, so the panel's weights and numbering are untouched.
- **The record checker asks two more questions** (`05-record.js`, so the form, the dashboard and the suite all ask them). *Their goals*: a match the opponents scored in with fewer lines under Their goals than their score (ten in the archive, named in the suite). *The written report*: only the labelled lines of a report's "Match details" block (Full-time, Player of the Match, Captain, Goalkeeper, Saves, Starting formation, Goalscorers) are compared with the record, people by surname so Dan matches Daniel; the prose is never read. It cannot catch a goal flagged as a penalty that the prose describes as open play. The match page prints their goals under ours ("Haydons Park: 67′ (pen)").
- **How to find a ground** (`grounds` in `src/data/venues.json`, `d.groundFor()` in `dataset.mjs`): the address and parking for a canonical venue, each with where it was read. The next-match card's Directions search uses the name plus the address, and the address and parking print under it (`.hx__ground`, hidden when empty, moved on by `10-home.js` with the card). First entry: Richardson Evans Memorial Playing Fields, from Full-Time. **A league table in an article has plain rows** (a `div`, not a link to `/league.html`), at the club's request; the home page's table still links.
- **The home page's next-match card knows match day** (`10-home.js`): on the fixture's date by London time it reads "Match day · today", counts hours, then "Under way" for two hours and "Result to follow" until somebody publishes. It carries a Directions link (a map search for the venue), re-pointed when the card moves to the next fixture.
- **An article can carry headings, sub-headings, tables and graphics** (`articleBody` in `news.mjs`, buttons in `control-writing.js`). `#`/`##` is a heading (h2) and `###` a sub-heading (h3, or an h2 styled smaller when no heading comes before it, so the level never skips). A table is a paragraph whose every line carries TABS (rows pasted from a document or spreadsheet) or sits between PIPES, first row the header, first column the row names, cells escaped before emphasis. `!![caption](address#WxH)` is a graphic: same address rule as a photograph, uploaded as PNG, shown whole and uncropped. `plainText` drops graphic lines and turns a table's tabs into ` · `. Plain lines are never guessed into headings: one marker, one meaning. **A table whose header is exactly Club | P | W | D | L | GF | GA | GD | Pts is drawn as the home page's `.tbl`** (crest, position, the club's row lit, every column in a hidden table for screen readers), from the rows the article wrote, so an article keeps its own week's table. The article page passes `d.badges`; without it the table stays plain. The row reveal in `10-home.js` runs on every `.tbl`: it looked for `#tbl` alone, which would have left an article's rows hidden under `html.js`. Articles written from here go in `src/data/articles-extra.json` and need `npm run covers` for their share card.
...
### League Eight as it is played

- **The table, the results and the League Eight fixtures are transcribed** into `src/data/league-eight-2627.json` from FA Full-Time, which refuses automated requests, so the club sends them and they are typed in. `tableAsOf` is the day the table was copied. Only League Eight fixtures are kept: another club's cup tie is not this club's match.
- **`npm run verify` checks the transcription against itself.** `compareDivision()` in `stats.mjs` derives the table from the results and compares every club, treating a club the results never mention as a row of noughts, because on the first weekend five of nine clubs have not played and `compareTable` alone would fail all five. A published row claiming a match nobody transcribed still fails, which is the typo it exists for. The suite runs the same function, with two probes.
...
- **While League Eight is played, the table, the results and the fixtures lead the home page**, in that order and whatever `home:layout` says (`LEAGUE_FIRST` / `leagueUnderway()` in `home-layout.mjs`, mirrored in the panel's preview from `SEED.leagueFirst`). The club asked for all three at the top and its saved record has them off, which only an admin login can change. The table band shows all nine clubs, and **Around League Eight** (`leagueNowBand` in `home.mjs`) sits directly under it with the round just played, the next round and the scorers. Before a ball is kicked the record decides alone, and a suite probe holds it to that. **The scorers are transcribed** from Full-Time's stat leaders page into `scorers` (goals only; cards need every page of that list), shown under the League Eight table on the league page, and `compareScorers()` in `stats.mjs` fails `npm run verify` if a club's scorers come to more goals than its results. **Assists** ride on each scorer (`assists`) and in their own `assisters` list, shown as a second table under the scorers (a man with assists and no goals would otherwise appear nowhere), ranked like the scorers. Where the club has asked Full-Time to correct an assist the file carries the club's figure and `_assistsNote` says so.
...
- **Every chart moves, and nothing beside a season's content is another season's.** After the draw the leading ring segment and the gauge glow slowly, a sheen crosses the columns and bars, bars grow one after another, and every chart figure counts up; all of it stops under reduced motion. On the player, stats and squad pages the charts are left to the scroll reveal on first load and replayed on every tab press (take `is-in` off, reflow, put it back); ring segments thicken and bars brighten on hover. The counts sitting outside the panels follow the tab: the heat map's positions (`data-pitch-counts`) and the stats table's players (`data-table-counts`). Career accolade chips in a player's hero say *All seasons*, and the pitch's SVG title carries no career figures.
- **A season's heat map shows that season's positions, and the tab script hides by attribute.** The layers are SVG `<g>` elements, which have no `hidden` property, so `el.hidden = true` hid nothing and every season's markers showed under every tab (Dean Knight's 26/27 pitch carried 25/26's wing-back beside his one right-back start). `[data-season-scope]` is toggled with `setAttribute('hidden')` now. The pitch has mown bands, full markings and a scan of light; the field blooms and the markers spring in again on every season press, and a wide player's heat runs long down the flank.
- **Player charts, redrawn.** Each season panel opens on **Share of the club's record**, a wheel (`profileWheel` in `player.mjs`) whose wedges are his figure as a percentage of the club's own total over the same matches: starts out of matches played, goals and goals-and-assists out of goals scored, assists out of every assist (the season's `d.playersBySeason`, or `d.players` for All seasons), wins out of the club's wins, clean sheets out of the club's clean sheets. It was a percentile against team-mates first and the club asked for the share instead. His photograph sits at the hub. **Starts, not appearances**, on the wheel and in the stats table's first column (`St`, and the per-start view divides by it): the club reads an appearance as starts plus bench outings, and the bench column already holds those. **The squad page and the player pages lead with starts too**: a player's opening sentence ("29 starts in 25/26, 2 off the bench, ..."), his first tile ("Starts, of 33 played"), the section and tab counts, the competition table (`St`, from the `starts` the profile now counts per competition), home and away (from the timeline's `started`), the squad comparison, and the meta description. A squad card reads Starts and Bench, Bench being every naming as a substitute (`subApps`), the same pair as the stats table; the squad chart is "Most starts". **Milestones and records count starts as well**, at the club's request: the milestone step is `starts` under the label "starts" (`MILESTONE_STEPS` in `stats.mjs`), the club record is "Most starts" in `clubRecords()`, on the records page ("Most starts", "First to 25 starts") and on the About page, and the programme's record chart and what-is-close list follow. The "Who turns up" home band lists starts too (it had been listing appearances under the unit "starts"); its key stays `appearances` because the club's saved running order names it that way. Appearances are still derived and still published where a page says appearances. **Match by match** is one `<g class="pf-col">` per match that grows in turn and names its match on hover; the results ring has a turning tick ring; the share is a 240-degree dial with a needle; home and away are mirrored bars. The running plot is revealed by a clip wipe (`pf-plot__wipe`), because a dash length measured in the stretched chart's own units stopped the line two thirds of the way along, and its dots and end are zero-length round-capped strokes so they stay round. The heaviest profile sits at 158KB of the 160KB ceiling: the heat circles lost their class and opacity attribute and the per-position match rows lost their indentation to pay for the wheel.
- **The squad page has "In pictures" too** (`picsBand` in `squad.mjs`, one `data-sq-chart-view` panel per season tab, switched in the squad script's `apply()`): players used by position (the hero's own "Players used" pool, which the suite holds them to), how the appearances came (from the start or off the bench), the six most used, and for a season who was new, retained or back (the derived statuses, read per season). Its CSS is in `32-squad.css`.
- **The squad page separates its seasons the same way.** It opens on the latest season with a competitive match played (`DEF` in `squad.mjs`), not the first tab, and everything drawn before the script runs is that tab's. The past players band follows the tab: its count is rewritten (`data-past-count`), and a season none of them played in hides the band and its Retired / Departed / Now on the staff sections instead of printing "17 in all" over nobody.
- **Player figures are league and cup together, never pre-season, and never split.** A league-only count with the cups on a "Cup ties" tab was built and taken down the same afternoon: the club does not want cup ties separated from the league. Pre-season appears on the results page and nowhere in player figures. The league page's "Not just the league" covers the latest season with a cup, leaves friendlies out and hides with that season's division tab.
- **A player's season tab is that season, and every season has its own tab.** `player.mjs` counted friendlies into the club's total ("1 of 7 played" for one League Eight match), and its opening sentence printed career totals dated to the current season, which is how 25/26 read as 26/27 on every profile. The totals are competitive only (league and cup, never pre-season), an **All seasons** tab sits last (`data-season-all`), and **a season tab shows that season and nothing else**: the opening sentence rides on `data-lede-<i>` and the tab script swaps it, and the bands counted over the whole career (against the squad, the detail, recognition) carry `data-season-scope="all"` and show only under All seasons. **The player stats page has "In pictures" too** (`picsBand` in `stats.mjs`, one `data-chart-view` panel per season view, switched with the leaders and the share): every match as goals scored above the line and conceded below, the results as a ring, the goals by position, and the six biggest goal-and-assist figures. Its CSS is in `34-stats.css`, not `home.css`, which is near its ceiling. **Each player page's season panel has "In pictures"**: a results ring (won, drawn, lost when playing), a share gauge (his part in the goals scored while he played, or clean sheet rate for a keeper), a home-and-away split, and match-by-match columns (season tabs only; under All seasons they would be the longest season again). Built at build time as inline SVG from the same profile as the figures, results told apart by shade of the one accent, animated on reveal under `html.js` with a reduced-motion rest, and held by the suite to agreeing with the record printed beside them. The All seasons panel drops the running-total plot unless more than one season has one, and drops "The last time out", both of which were the latest season again. **The heat map follows the tab, in one band.** Each season's heat is built from that season's team sheets - a start once per position named, a bench place half - and drawn as its own `<g data-season-scope="i">` layer on one pitch, with All seasons carrying the career layer (a player with one season has one layer scoped `"i all"`). The position list is one list whose rows carry every tab's count and bar width in `data-heat`, and each match row is written once, tagged with its season. Three separate bands put a profile at 207KB, most of it the same match rows twice, and the interaction script only ever wired the first. The page opens on the latest season the player played in, or All seasons if he played in none; with the script blocked everything shows.
- **Goal clips the club sends are filed in `src/data/match-clips.json`**, against a match id and the goal's place in that match's own goal list, with the web encodes in `/assets/video` (1080p H.264, `+faststart`, from the club's 4K HEVC originals). The panel's clip slots write to the database, which a developer machine cannot, so this is the same device as `fixtures-2627.json`. `d.goalFilmFor(m)` resolves each clip's scorer and assist from the record, never from the file, and the match page shows each one captioned "Goal 1 · scorer, assisted by ..." under the result's cover (`poster`, `preload="none"`), anchored `#goal-N`; the videos page lists them linking there. First filed: both goals of the 2-1 against Three Little Birds (Jon Lloyd from Charlie Dunkley, then Charlie Dunkley from Jon Lloyd). The suite checks every filed clip against its page and the file on disk.
- **The registered squad is the club's FA registration list**, transcribed in `src/data/registered-2627.json` (names as registered: Jack Bridges, Robert Heath, Ade Owolana). The sponsors page's "Sponsor a player's season" shelf reads it rather than the squad record, links each name by `num`, and leaves the manager off (`sponsorship: false`). On 12 September 2026 the club's record was brought into line with it through the panel: Louis Allen, Danny Cain, George Churchill and Samuel Nichols added (42-45), Jim El Bayati, Richard Epathite, Danny Jackson, David Jones, Daniel Rand, Kafele Brown, Alfie James Inman and Malachi Mullings set In the squad for 26/27, Harry Pyke Left the club, the three renames made, and the trialist entries for Liam Alford and Liam Kirby removed once the 2 August friendly credited Liam Kirby's signed record. **A player with no status entry and no 26/27 team sheet reads as "Not at the club" and the Squad screen offers no dropdown for him**, so that state has to be written to `roster:status` directly.
- **Stated positions group by the first position named, midfield before defence.** The club gave a position for every registered player (positions-extra.json for those without a panel record, the record itself for the rest). The old rule tested "def" first, so "Defensive midfielder" filed a holding midfielder with the defenders.
- **The home page tab reads "League Eight 26/27 · Sue's Angels FC"** once the division has a match played (`homeMeta` in `build.mjs`), overriding the recovered "League Ten Champions" title; before that the recovered title stands.
- **The hero and ticker quote the title season's record**, not every league match: from the first League Eight result they read "19 wins from 19 · League Ten Champions 25/26".
- **A cover wears the opponent's badge through the site's resolver.** `make-covers.mjs` and the panel looked a badge up by exact stored name, so eight cards (BPR Men's, Woking Veterans Sundays...) wore the club crest twice. The seed now keys every opponent under its stored name.
- **The player stats page says why a new season is not showing.** It opens on the newest season with figures, and figures come from team sheets, so a result saved without one leaves it on last season. It now says which season has started and that its team sheet is what switches it over.

### The matchday squad is the eleven and the bench

Enter result fills the team sheet from the squad picked on the Matchday screen, **first eleven in, the rest on the bench**, and the screen called all sixteen "Player N", so a keeper picked in slot 14 went onto the sheet as a substitute. The slots are **Starter 1-11 and Substitute 1-5** now, and `squadStarters` is saved beside `squad` so a blank in the eleven never promotes a substitute; the match form reads it first and falls back to eleven for a squad saved before it existed. The dropdowns are grouped by position in surname order, a player picked in one slot is greyed out in the others, a line underneath counts the eleven and the bench and says when there is no goalkeeper in it, and **Copy for WhatsApp** puts the fixture, the eleven and the substitutes on the clipboard, with a box to copy from where the browser refuses. The checklist no longer shows two "after the match" rows before kick-off. Switching match used to re-run the screen on the same element, stacking a click handler each time so one Save could write to two fixtures; it redraws the panel fresh.
...
- **The milestone step and its label name the same figure, and that figure is now starts.** It once counted starts under the label "appearances", which were the same number until a substitute the record can prove was on the pitch started counting, and the band told the front page that Charlie Dunkley was one appearance from 25 when he had already played 25. It read `apps` after that; since September 2026 the club counts milestones in starts, so the key is `starts` and the label says starts.
...
11. **An HTML comment in a template ships.** 223KB of them did, across 124 pages: the footer's note on every page, and a paragraph per squad card, which was 59KB of `squad.html` alone. Comments in templates are JS comments inside the literal, `${/* ... */''}`, and the build output holds no `<!--` at all. JS and CSS were already minified; HTML never was. Player pages also drop their source indentation (`playerPage` body), because a panel per season repeats it.
12. **A check run on the live snapshot, not only the committed one.** The pre-push hook builds the committed `recovered-live.json`, which is weeks behind the database, so thirty failures that only the synced data could produce passed every push. `npm run sync` first when looking for bugs. Four of them were checks that broke on real data having caught nothing: the league-first check sliced three places when only two leaders were published, the matchday checks needed a fixture in the database (they craft one now), and the squad-number check paired the last figure of one row with the name opening the next (it breaks lines at `li`, `br`, `p`, `div`, `tr` and headings now, with probes both ways). The rest were real: the programme numbered its goals beside the scorer ("6 Elis Brumpton"; the disc holds the minute now), a player with no games in a season showed "0% · 0 of 0" won, the panel counted a build card behind a stored cover as drawn, and the home page's rotating rail mark put "The Reeves, Hanworth" beside an away fixture.
