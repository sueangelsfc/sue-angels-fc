...
- **Player figures are league figures, at the club's request.** The player pages' season panels, All seasons tab and opening sentence count league matches only, with a **Cup ties** tab of their own (`data-season-cups`); the player stats page's leaders, table, tallies and scorers count the league, its first chip reads **League**, and each cup keeps a chip. Pre-season appears on the results page and nowhere in either. The league page's "Not just the league" covers the latest season with a cup, leaves friendlies out and hides with that season's division tab. Squad cards, milestones and records still count competitive matches.
- **A player's season tab is that season, and every season has its own tab.** `player.mjs` counted friendlies into the club's total ("1 of 7 played" for one League Eight match), and its opening sentence printed career totals dated to the current season, which is how 25/26 read as 26/27 on every profile. The totals are competitive only, the sentence gives this season and then every season, an **All seasons** tab sits last (`data-season-all`, never the one the page opens on), and the bands counted over the whole career (against the squad, on the pitch, the detail, the squad accolades) say *All seasons* instead of naming the current one.
