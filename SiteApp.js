...
/* Home sponsor strip - quiet "Proudly backed by" logo row. Reads live sponsors
   from SponsorsStore (admin-managed) and falls back to the founding partners. */
function HomeSponsorStrip() {
  var h = React.createElement;
  var defaults = [{ name: 'Sporting Solutions Ltd', logo: 'assets/sponsors/sporting-solutions.webp' }, { name: 'Hodgson Roofing', logo: 'assets/sponsors/hodgson-roofing.webp' }, { name: 'Staines Rugby Club', logo: 'assets/sponsors/staines-rugby.png' }];
  var custom = (window.SponsorsStore && window.SponsorsStore.list) ? window.SponsorsStore.list() : [];
  var seen = {}, list = [];
  custom.concat(defaults).forEach(function (s) { if (s && s.name && s.logo && !seen[s.name]) { seen[s.name] = 1; list.push(s); } });
  list = list.slice(0, 6);
  if (!list.length) return null;
  return h("section", { className: "mh-backers" }, h("div", { className: "m-wrap" }, h("div", { className: "mh-backers__row m-glass" },
    h("p", { className: "m-eyebrow m-eyebrow--volt mh-backers__eb" }, "Proudly backed by"),
    h("div", { className: "mh-backers__logos" }, list.map(function (s, i) { return h("span", { className: "mh-backers__pill", key: i, title: s.name }, h("img", { src: s.logo, alt: s.name, loading: "lazy" })); })),
    h("a", { className: "mh-backers__cta", href: "sponsors.html" }, "Partner with us →"))));
}
function maHeart() {
