...
function Contact() {
  ...
  return (<React.Fragment>
    ...
    <section className="mp-sec"><div className="m-wrap"><div className="mp-split">
      <div>{routes.map(([k, l, s]) => <button key={k} className={`mp-route ${route === k ? 'is-active' : ''}`} onClick={() => { setRoute(k); setSent(false); }}><b>{l}</b><span>{s}</span></button>)}<div className="m-glass" style={{ padding: 18, marginTop: 8 }}><p className="m-eyebrow" style={{ marginBottom: 10 }}>Direct line</p><p style={{ color: 'var(--m-ink-2)', font: '600 0.9rem var(--m-sans)', lineHeight: 1.8 }}>susangelsfc@gmail.com<br />@suesangelsfc · Instagram</p></div></div>
