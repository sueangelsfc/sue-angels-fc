...
const pdate = (s) => {
  ...
  return m ? new Date(2e3 + +m[3], MON[m[2]] || 0, +m[1]) : /* @__PURE__ */ new Date(0);
};
...
function setPieceStats(season) {
  ...
  if (season && season !== "all") {
    var sOf = window.seasonOf || function() {
      return window.CURRENT_SEASON;
    };
    var byId = {};
    (window.getDerivedResults ? window.getDerivedResults() : []).forEach(function(r) {
      byId[r.id] = r;
    });
    entries = entries.filter(function(e) {
      var r = byId[e.id];
      return r ? (sOf(r) || window.CURRENT_SEASON) === season : false;
    });
  }
  ...
  entries.forEach(function(e) {
    ...
    (data.goals || []).forEach(function(g) {
      if (g.type === "pen" || g.penalty) penG++;
        ...
        if (g.setType === "corner") cornerG++;
        else if (g.setType === "freekick") fkG++;
        else if (g.setType === "throwin") throwG++;
        else otherSetG++;
      } else openG++;
    });
    (data.assists || []).forEach(function(a) {
      var s = a.source || "open";
      if (s === "corner") cornerA++;
      else if (s === "freekick") fkA++;
      else if (s === "throwin") throwA++;
      else openA++;
    });
  });
  var setG = cornerG + fkG + throwG + otherSetG;
  return { openG, cornerG, fkG, throwG, penG, setG, openA, cornerA, fkA, throwA, totalG: openG + setG + penG, totalA: openA + cornerA + fkA + throwA };
}
function SetPiecePanel(props) {
  ...
  return h(
    ...
    h("div", { className: "mp-sp__grid" }, rows.map(function(r, i) {
      return h(
        "div",
        { className: "mp-sp__tile", key: i },
        h("span", { className: "mp-sp__l" }, r[0]),
        h(
          "div",
          { className: "mp-sp__nums" },
...
function TeamStatsPanel(props) {
  ...
  var usRow = isCurrent ? (window.RAW_TABLE || []).find(function(x) {
    return x.us;
  }) : null;
  var champions = !!(usRow && usRow.p === 1);
  var unbeatenLeague = !!(usRow && usRow.l === 0 && usRow.pl > 0);
  var MON2 = { Jan: 0, Feb: 1, Mar: 2, Apr: 3, May: 4, Jun: 5, Jul: 6, Aug: 7, Sep: 8, Oct: 9, Nov: 10, Dec: 11 };
  var pdate2 = function(s) {
    var m = /(\d{1,2})\s+(\w{3})\s+(\d{2,4})/.exec(s || "");
    return m ? new Date(+m[3] < 100 ? 2e3 + +m[3] : +m[3], MON2[m[2]] || 0, +m[1]).getTime() : 0;
  };
  var sOf2 = window.seasonOf || function() {
    return window.CURRENT_SEASON;
  };
  var results = (window.getDerivedResults ? window.getDerivedResults() : []).slice().filter(function(r) {
    return season === "all" || (sOf2(r) || window.CURRENT_SEASON) === season;
  }).sort(function(a, b) {
    return pdate2(b.date) - pdate2(a.date);
  }).slice(0, 8);
  var form = results.map(function(r) {
    ...
    if (r.kind === "walkover") {
      res = "w";
      score = "W/O";
    } else if (us == null || th == null) {
      return null;
    } else {
      score = us + "–" + th;
      if (r.kind === "penalty" && r.pens) {
        var uP = uh ? r.pens.hs : r.pens.as, tP = uh ? r.pens.as : r.pens.hs;
        res = uP > tP ? "w" : "l";
      } else res = us > th ? "w" : us === th ? "d" : "l";
    }
    return { res, score, opp: (uh ? r.away : r.home).replace(" FC", "") };
  }).filter(Boolean).reverse();
  var gTot = tt.gf + tt.ga || 1;
  var legend = function(cls, t) {
    return h("span", null, h("i", { className: cls }), t);
  };
  return h(
    "div",
    { className: "ts" },
    h(
      ...
      h(
        ...
        champions || unbeatenLeague ? h(
          ...
          champions ? h("span", { className: "ts-badge", key: "p" }, "Promoted") : null
        ) : null
      )
    ),
    h(
...
function seasonCompTotals(season) {
  ...
  var results = (window.getDerivedResults ? window.getDerivedResults() : []).filter(function(r2) {
    return (sOf(r2) || window.CURRENT_SEASON) === season;
  });
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    if (them === 0) cs++;
    if (r.kind === "penalty" && r.pens) {
      var uP = usHome ? r.pens.hs : r.pens.as, tP = usHome ? r.pens.as : r.pens.hs;
      if (uP > tP) w++;
      else l++;
    } else if (us > them) w++;
    else if (us === them) d++;
...
function matchLog(num, compMatcher, seasonKey) {
  ...
  if (seasonKey && seasonKey !== "all") {
    const sOf = window.seasonOf || (() => window.CURRENT_SEASON);
    results = results.filter((r) => (sOf(r) || window.CURRENT_SEASON) === seasonKey);
  }
...
function ProfileCard({
  ...
  const data = useMemo(() => {
    ...
    const p2 = squad.find((x) => x.num === num) || squad[0];
    if (!p2) return null;
    const log2 = matchLog(p2.num, matcher, seasonKey);
    const teamPlayed2 = (window.getDerivedResults ? window.getDerivedResults() : []).filter((r) => {
      if (matcher && !matcher(r.competition)) return false;
      if (seasonKey && seasonKey !== "all") {
        const sOf = window.seasonOf || (() => window.CURRENT_SEASON);
        if ((sOf(r) || window.CURRENT_SEASON) !== seasonKey) return false;
      }
      return r.kind !== "walkover";
    }).length;
    const wins2 = log2.filter((m) => m.res === "w").length;
    const teamGf2 = (typeof seasonCompTotals === "function" ? seasonCompTotals(season).gf : 0) || 0;
    const goalShare2 = teamGf2 ? Math.round(p2.goals / teamGf2 * 100) : 0;
    const rankIn = (key) => squad.slice().sort((a, b) => (b[key] || 0) - (a[key] || 0)).findIndex((x) => x.num === p2.num) + 1;
    const comps2 = (window.COMPETITIONS || []).filter((c) => c.key !== "all").map((c) => ({
      label: c.label,
      s: window.derivedPlayerStats(p2.num, c.match, seasonKey)
    })).filter((c) => c.s.apps > 0);
    const open2 = p2.goals - p2.penaltiesScored - p2.setPiecesScored;
    const isGK2 = !!(p2.gk || typeof posGroup === "function" && posGroup(p2.mostPlayedPosition, p2.gk) === "Goalkeepers");
    return {
      p: p2,
      log: log2,
      teamPlayed: teamPlayed2,
      wins: wins2,
      photo: photoOf(p2.num),
      goalShare: goalShare2,
      teamGf: teamGf2,
      open: open2,
      comps: comps2,
      isGK: isGK2,
  ...
  return /* @__PURE__ */ React.createElement(
    ...
    }, p.last, /* @__PURE__ */ React.createElement("span", null, p.first)))),
    /* @__PURE__ */ React.createElement("div", {
    ...
    /* @__PURE__ */ React.createElement("div", {
      ...
      className: "m-bar__val m-num"
    }, p.goals, "·", p.assists)), /* @__PURE__ */ React.createElement("div", {
      className: "m-bar__track"
    }, /* @__PURE__ */ React.createElement("div", {
      className: "m-bar__fill m-bar__fill--soft",
      style: {
    ...
    /* @__PURE__ */ React.createElement("div", {
      ...
      const heat = posMax ? n / posMax : 0;
      const size = 80 + heat * 135;
      return /* @__PURE__ */ React.createElement("span", { className: "m-pitch__heat", key: "h" + i, style: { left: xy[0] + "%", top: xy[1] + "%", width: size, height: size, "--heat": heat } });
    }), posTop.map(([pos, n], i) => {
      const xy = POS_XY[pos] || [50, 50];
      ...
      }, pos, /* @__PURE__ */ React.createElement("small", null, n, "×"));
    })))),
    comps.length ? /* @__PURE__ */ React.createElement("div", {
      className: "m-panel"
    }, /* @__PURE__ */ React.createElement("p", {
      className: "m-panel__t"
    }, "By competition · 25/26"), /* @__PURE__ */ React.createElement("table", {
      className: "m-split"
      ...
      className: "m-split__hl"
    }, c.s.assists), /* @__PURE__ */ React.createElement("td", null, c.s.motm)))))) : null,
    /* @__PURE__ */ React.createElement("div", {
      className: "m-panel"
    }, /* @__PURE__ */ React.createElement("p", {
      className: "m-panel__t"
    ...
    function() {
      ...
      rec.records.forEach(function(r) {
        items.push(["record", r.title, r.landmark ? r.scope || r.value : r.value]);
      });
      rec.potm.forEach(function(r) {
        items.push(["award", "Player of the Month", ((r.month || "") + " " + (r.season || "")).trim()]);
      });
      rec.seasonAwards.forEach(function(r) {
        items.push(["award", r.title, r.season || ""]);
      });
      var ms = rec.milestones || [];
      if (!items.length && !ms.length) return null;
      return h(
        "div",
        { className: "m-panel m-pc__ach" },
        h("p", { className: "m-panel__t" }, "Achievements & Milestones"),
        items.length ? h("div", { className: "pc-ach__list" }, items.map(function(it, i) {
          return h("div", { key: "a" + i, className: "pc-ach__item pc-ach__item--" + it[0] }, h("span", { className: "pc-ach__label" }, it[1]), it[2] ? h("span", { className: "pc-ach__sub" }, it[2]) : null);
        })) : null,
        ms.length ? h("div", { className: "pc-ach__chips" }, ms.map(function(m, i) {
          return h("span", { key: "m" + i, className: "pc-ach__chip" }, m.title);
        })) : null
      );
    }()
  );
...
function ResCard({
  ...
  }, r.competition || "League Ten", " · ", usHome ? "Home" : "Away"));
}
function saSlug(s) {
  return String(s || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
}
function saPlayerName(p) {
  return p ? ((p.first || "") + " " + (p.last || "")).trim() : "";
}
  ...
  return new Promise(function(resolve) {
    try {
      ...
      var draw = function() {
        ...
        bg0.addColorStop(0, "rgb(20,56,73)");
        bg0.addColorStop(1, "rgb(4,18,27)");
        c.fillStyle = bg0;
        c.fillRect(0, 0, W, H);
        var tracked = function(v) {
          try {
            c.letterSpacing = v;
          } catch (e) {
          }
        ...
        Promise.all([load(spec.badge || "assets/badge/sue-angels-badge.webp"), load(spec.photo), load(spec.homeBadge), load(spec.awayBadge), load("assets/sponsors/sporting-solutions.webp"), load("assets/sponsors/hodgson-roofing.webp"), load("assets/sponsors/staines-rugby.png")]).then(function(imgs) {
          ...
          var clean = function(s) {
            return String(s || "").replace(/ FC$/i, "").trim();
          };
          c.textAlign = "center";
          ...
          var drawFooter = function() {
            if (sponsors.length) {
              ...
              c.font = '700 25px "Hanken Grotesk", Arial, sans-serif';
              c.fillText("PROUDLY BACKED BY", W / 2, SP_LABEL_Y);
              tracked("0px");
            ...
            c.fillStyle = "rgba(255,255,255,0.55)";
            c.font = '600 34px "Hanken Grotesk", Arial, sans-serif';
            c.fillText(String(spec.footer || "suesangelsfc.co.uk"), W / 2, FOOT_Y);
          };
          if (spec.kind === "score") {
            var drawBadge = function(im, cx, cyc, sz) {
              if (!im) return;
              var ar2 = im.width / im.height, w = sz, hh = sz;
              if (ar2 > 1) hh = sz / ar2;
              else w = sz * ar2;
              c.drawImage(im, cx - w / 2, cyc - hh / 2, w, hh);
            };
            ...
            c.font = '700 28px "Hanken Grotesk", Arial, sans-serif';
            c.fillText("FULL TIME", W / 2, SC.ftY);
            tracked("0px");
            ...
            c.fillStyle = "#FFFFFF";
            c.fillText(scoreStr, W / 2, SC.scY + SC.scFont * 0.34);
            c.fillStyle = "rgba(255,255,255,0.62)";
            c.font = '600 26px "Hanken Grotesk", Arial, sans-serif';
            ...
            tracked("4px");
            c.fillStyle = "#D6F23A";
            c.font = '700 27px "Hanken Grotesk", Arial, sans-serif';
            c.fillText(String(spec.eyebrow || "").toUpperCase(), W / 2, cy);
            tracked("0px");
            cy += 28;
            if (spec.result) {
              ...
              c.fillStyle = pbg;
              rr(prx, pry, ppw, pph, pph / 2);
              c.fill();
              c.fillStyle = ptx;
              c.fillText(rl, W / 2, pry + 38);
              tracked("0px");
              cy = pry + pph + 50;
            } else {
              cy += 42;
            ...
            var meta = [];
            if (spec.venue) meta.push(spec.venue);
            if (spec.kick) meta.push("KO " + spec.kick);
            if (meta.length && !post) {
              tracked("3px");
              c.fillStyle = "rgba(255,255,255,0.5)";
              c.font = '600 25px "Hanken Grotesk", Arial, sans-serif';
              c.fillText(meta.join("   ·   ").toUpperCase(), W / 2, cy);
              tracked("0px");
              cy += 64;
            }
            cy += 8;
            ...
            var section = function(label, items) {
              ...
              tracked("4px");
              c.fillStyle = "#D6F23A";
              c.font = "700 " + labelFont + 'px "Hanken Grotesk", Arial, sans-serif';
              c.fillText(label, W / 2, cy);
              tracked("0px");
              cy += lineH;
              c.fillStyle = "#FFFFFF";
              c.font = "700 " + nameFont + 'px "Hanken Grotesk", Arial, sans-serif';
              items.forEach(function(it) {
                c.fillText(String(it.name).toUpperCase() + (it.n > 1 ? "  ×" + it.n : ""), W / 2, cy);
                cy += lineH;
              });
              cy += lineH * 0.6;
            };
            ...
            tracked("4px");
            c.fillStyle = "rgba(255,255,255,0.5)";
            c.font = "700 " + (post ? 23 : 26) + 'px "Hanken Grotesk", Arial, sans-serif';
            c.fillText(String(spec.eyebrow || "LEAGUE TABLE").toUpperCase(), W / 2, post ? 104 : 150);
            tracked("0px");
            c.fillStyle = "#D6F23A";
            c.font = "700 " + (post ? 54 : 68) + 'px "Clash Display", "Hanken Grotesk", Arial, sans-serif';
            ...
            var ML = 54, MR = 1026;
            c.font = "700 " + headFont + 'px "Hanken Grotesk", Arial, sans-serif';
            c.fillStyle = "rgba(255,255,255,0.5)";
            tracked("1px");
            c.textAlign = "left";
            c.fillText("#", COL.pos, topY);
            c.fillText("CLUB", COL.club, topY);
            c.textAlign = "right";
            c.fillText("P", COL.p, topY);
            c.fillText("W", COL.w, topY);
            c.fillText("D", COL.d, topY);
            c.fillText("L", COL.l, topY);
            c.fillText("GD", COL.gd, topY);
            c.fillText("PTS", COL.pts, topY);
            tracked("0px");
            c.strokeStyle = "rgba(255,255,255,0.14)";
            c.lineWidth = 2;
            c.beginPath();
            c.moveTo(ML, topY + rh * 0.34);
            c.lineTo(MR, topY + rh * 0.34);
            c.stroke();
            var ty = topY + rh;
            trows.forEach(function(r) {
              if (r.us) {
                c.fillStyle = "rgba(214,242,58,0.14)";
                rr(ML, ty - rh * 0.64, MR - ML, rh * 0.9, 12);
                c.fill();
              }
              var main = r.us ? "#D6F23A" : "#FFFFFF";
              c.textAlign = "left";
              c.font = "800 " + rowFont + 'px "Hanken Grotesk", Arial, sans-serif';
              c.fillStyle = r.us ? "#D6F23A" : "rgba(255,255,255,0.55)";
              c.fillText(String(r.pos), COL.pos, ty);
              c.fillStyle = main;
              c.font = (r.us ? "800 " : "600 ") + rowFont + 'px "Hanken Grotesk", Arial, sans-serif';
              ...
              c.textAlign = "right";
              c.fillStyle = r.us ? "#D6F23A" : "rgba(255,255,255,0.82)";
              c.font = "600 " + rowFont + 'px "Hanken Grotesk", Arial, sans-serif';
              c.fillText(String(r.pl), COL.p, ty);
              c.fillText(String(r.w), COL.w, ty);
              c.fillText(String(r.d), COL.d, ty);
              c.fillText(String(r.l), COL.l, ty);
              c.fillText(typeof r.gd === "number" ? (r.gd > 0 ? "+" : "") + r.gd : String(r.gd), COL.gd, ty);
              c.fillStyle = main;
              c.font = "800 " + rowFont + 'px "Hanken Grotesk", Arial, sans-serif';
              c.fillText(String(r.pts), COL.pts, ty);
              ty += rh;
            });
            c.textAlign = "center";
            drawFooter();
            ...
            tracked("6px");
            c.fillStyle = "rgba(255,255,255,0.5)";
            c.font = "700 " + (post ? 26 : 29) + 'px "Hanken Grotesk", Arial, sans-serif';
            c.fillText(String(spec.eyebrow || "SUE'S ANGELS FC").toUpperCase(), W / 2, HD.ey);
            tracked("0px");
            var SUB_Y = SP_LABEL_Y - 66, TITLE_Y = SUB_Y - 56;
            var px2 = 130, py2 = HD.py, pw2 = W - 260, ph2 = TITLE_Y - 132 - py2;
            c.save();
            rr(px2, py2, pw2, ph2, 44);
            c.clip();
            if (photo) {
              ...
              c.fillStyle = P.frame;
              c.fillRect(px2, py2, pw2, ph2);
              if (badge) {
                var bz = Math.min(340, ph2 - 80);
                c.drawImage(badge, W / 2 - bz / 2, py2 + ph2 / 2 - bz / 2, bz, bz);
              }
            }
            var sg = c.createLinearGradient(0, py2 + ph2 - 200, 0, py2 + ph2);
            sg.addColorStop(0, "rgba(" + P.scrim + ",0)");
            sg.addColorStop(1, "rgba(" + P.scrim + ",0.5)");
            c.fillStyle = sg;
            c.fillRect(px2, py2 + ph2 - 200, pw2, 200);
            c.restore();
            c.strokeStyle = "rgba(214,242,58,0.45)";
            c.lineWidth = 3;
            rr(px2, py2, pw2, ph2, 44);
            c.stroke();
            c.fillStyle = P.title;
            c.font = "700 " + (post ? 64 : 74) + 'px "Clash Display", "Hanken Grotesk", Arial, sans-serif';
            saWrapText(c, String(spec.title || "").toUpperCase(), W / 2, TITLE_Y, W - 150, post ? 72 : 82, 2);
            if (spec.subtitle) {
              tracked("2px");
              c.fillStyle = "#D6F23A";
              c.font = "700 " + (post ? 28 : 31) + 'px "Hanken Grotesk", Arial, sans-serif';
              c.fillText(String(spec.subtitle).toUpperCase(), W / 2, SUB_Y);
              tracked("0px");
            }
            drawFooter();
          }
...
window.saReportStorySpec = function(report) {
  try {
    ...
    var nameOf = function(num) {
      var p = (window.SQUAD || []).filter(function(x) {
        return x.num === num;
      })[0];
      return p ? ((p.first ? p.first + " " : "") + p.last).trim() : "Sue's Angels";
    };
...
function ShareBtn({ title, what, label, image, url: urlProp, story }) {
  ...
  var shareUrl = function() {
    try {
      return urlProp ? new URL(urlProp, location.href).href : location.href;
    } catch (e) {
  ...
  var track = function(method) {
    if (window.saTrack) window.saTrack("share", { what: what || "page", method });
  };
  var shareTitle = function() {
  ...
  var copyLink = async function(method, msg) {
    ...
    try {
      ...
      track(method || "copy");
      alert(msg || "Link copied. Paste it anywhere you like - Instagram, a story, a chat.");
    } catch (e) {
  ...
  var shareImageFile = async function() {
    ...
    try {
      if (story) {
        var spec = typeof story === "function" ? story() : story;
        if (spec) {
          var gen = await saStoryImage(spec, fmt === "post");
          if (gen) src = gen;
        }
      }
    } catch (e) {
  ...
  var toApp = async function(method, copyMsg) {
    ...
    copyLink(method, copyMsg);
  };
  var openWin = function(u2, method) {
    window.open(u2, "_blank", "noopener,noreferrer");
    track(method);
    setOpen(false);
  };
  ...
  var shareToIg = function() {
    try {
      if (navigator.clipboard) navigator.clipboard.writeText(shareUrl());
    } catch (e) {
    }
    toApp("instagram", igMsg);
  };
  var items = [
    navigator.share ? { k: "native", label: "Share to apps…", fn: nativeShare } : null,
    { k: "ig", label: "Instagram", fn: shareToIg },
    { k: "x", label: "X (Twitter)", fn: function() {
      openWin("https://twitter.com/intent/tweet?text=" + t + "&url=" + u, "x");
    } },
    { k: "tt", label: "TikTok", fn: function() {
      toApp("tiktok", ttMsg);
    } },
    { k: "wa", label: "WhatsApp", fn: function() {
      openWin("https://wa.me/?text=" + t + "%20" + u, "whatsapp");
    } },
    { k: "fb", label: "Facebook", fn: function() {
      openWin("https://www.facebook.com/sharer/sharer.php?u=" + u, "facebook");
    } },
    { k: "copy", label: "Copy link", fn: function() {
      copyLink("copy");
    } }
  ].filter(Boolean);
  return h("div", { className: "sa-share", ref }, h("button", { type: "button", className: "m-btn m-btn--ghost", "aria-haspopup": "menu", "aria-expanded": open ? "true" : "false", onClick: function() {
    setOpen(function(v) {
      return !v;
    });
  }, style: { display: "inline-flex", alignItems: "center", gap: 8 } }, label || "Share", h("svg", { viewBox: "0 0 24 24", width: 15, height: 15, fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": "true" }, h("path", { d: "M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8" }), h("polyline", { points: "16 6 12 2 8 6" }), h("line", { x1: 12, y1: 2, x2: 12, y2: 15 }))), open ? h("div", { className: "sa-share__menu", role: "menu" }, (story ? [h("div", { key: "fmt", style: { display: "flex", gap: 6, padding: "8px 10px 10px" }, onClick: function(e) {
    e.stopPropagation();
  } }, [["story", "Story · 9:16"], ["post", "Post · 4:5"]].map(function(f) {
    return h("button", { key: f[0], type: "button", onClick: function() {
      setFmt(f[0]);
    }, style: { flex: 1, padding: "7px 8px", borderRadius: 8, border: "1px solid " + (fmt === f[0] ? "var(--m-volt)" : "var(--m-edge-2)"), background: fmt === f[0] ? "var(--m-volt)" : "transparent", color: fmt === f[0] ? "var(--m-navy)" : "var(--m-ink-2)", font: "600 0.72rem var(--m-sans)", cursor: "pointer" } }, f[1]);
  }))] : []).concat(items.map(function(it) {
    return h("button", { key: it.k, type: "button", role: "menuitem", className: "sa-share__item", onClick: it.fn }, it.label);
  })).concat(story ? [h("div", { key: "ighint", style: { padding: "9px 12px 5px", marginTop: 4, borderTop: "1px solid var(--m-edge)", font: "500 0.64rem/1.45 var(--m-sans)", color: "var(--m-ink-3)" } }, "Instagram Story link: add the Link sticker and paste — your link is auto-copied.")] : [])) : null);
}
function Home({
  go
}) {
  ...
  return /* @__PURE__ */ React.createElement(React.Fragment, null, h("section", { className: "cine" }, h("div", { className: "cine__photo" }, h("img", { ref: imgRef, src: "assets/hero-team.webp", fetchpriority: "high", alt: "", onError: function(e) {
    if ((e.target.getAttribute("src") || "").indexOf("hero-team.webp") > -1) e.target.src = "assets/hero-team.jpg";
  } })), h("div", { className: "cine__grad" }), h("div", { className: "m-wrap cine__inner", ref: dRef }, h("div", { className: "cine__copy" }, h("p", { className: "m-eyebrow m-eyebrow--volt" }, "Southern Sunday Football League"), h("h1", { className: "cine__title" }, "Sue's Angels ", h("em", null, "FC")), h("div", { className: "cine__meta" }, h("span", null, "League Ten"), h("i", { "aria-hidden": "true" }), h("span", null, "Season 25/26"), h("i", { "aria-hidden": "true" }), h("span", null, t.l === 0 ? "Champions, Unbeaten" : "Champions")), h("p", { className: "cine__lead" }, "Founded in memory of Susan Anne Martin. Played ", t.pl, ", won ", t.w, ", and crowned champions of League Ten in our very first season."), h("div", { className: "cine__cta" }, h("button", { className: "m-btn m-btn--volt", onClick: function() {
    go("schedule");
  } }, "View fixtures ", h(Arrow, null)), h("a", { className: "m-btn m-btn--ghost", href: "join.html" }, "Join the club")))), h("div", { className: "cine__strip" }, next || session ? h("div", { className: "cine__next2" + (next ? " cine__next2--match" : "") }, I.cal, next ? h("span", { className: "cine__next2__match" }, window.TeamBadge ? h(window.TeamBadge, { team: next.home, size: 20 }) : null, h("b", { className: "cine__next2__team" }, (next.home || "").replace(/ FC$/, "")), h("span", { className: "cine__next2__vs" }, "v"), h("b", { className: "cine__next2__team" }, (next.away || "").replace(/ FC$/, "")), window.TeamBadge ? h(window.TeamBadge, { team: next.away, size: 20 }) : null) : h("span", { className: "cine__next2__txt" }, "Next session · " + session.dayName + " " + session.dateStr + " · " + session.timeStr), next ? h("span", { className: "cine__next2__txt cine__next2__when" }, "Kick-off " + (next.date || "") + (next.kick ? " · " + next.kick : "") + (next.competition ? " · " + next.competition : "")) : null, cd ? h("b", null, (cd.d > 0 ? cd.d + "D " : "") + pad(cd.h) + "H " + pad(cd.m) + "M") : null, h("button", { className: "cine__next2__cta", onClick: function() {
    go("schedule");
  } }, "View fixtures ", h(Arrow, null))) : null, h("div", { className: "cine__selrow" }, h("span", { className: "cine__rule" }), h("span", { className: "cine__sel" }, "Media News"), h("span", { className: "cine__rule" })), function() {
    var arts = (window.getCustomArticles ? window.getCustomArticles() : []).filter(function(a) {
      return a && a.title;
    });
    arts.sort(function(a, b) {
      return (b.sortISO ? Date.parse(b.sortISO) : pdate(b.date || "")) - (a.sortISO ? Date.parse(a.sortISO) : pdate(a.date || ""));
    });
    var recent = arts.slice(0, 3);
    return recent.length ? h("div", { className: "cine__thumbs" }, recent.map(function(a, i) {
      var pc = window.getPostCover ? window.getPostCover(a.id) : null;
      var ic = (window.getArticleCover ? window.getArticleCover(a.id) : null) || a.cover || null;
      var coverEl = pc ? maGenCover(pc) : ic && typeof ic === "string" ? h("div", { className: "mp-news__cover" }, h("img", { src: ic, alt: "", loading: "lazy" })) : maGenCover({ layout: "badges", top: "", left: "assets/badge/sue-angels-shield.webp", right: "", center: "", bottom: "" });
      return h("button", { key: a.id || i, className: "cine__thumb cine__thumb--news mp-clickable", onClick: function() {
        location.href = "news.html?article=" + (a.id || "");
      } }, coverEl, h("div", { className: "cine__tlabel cine__tlabel--news" }, h("span", { className: "cine__tag" }, a.cat || "News"), h("b", null, a.title)));
    })) : null;
  }())), h(
    "section",
    { className: "mh-sec" },
    h("div", { className: "m-wrap" }, h("div", { className: "mh-ocamba" }, h("div", { className: "mh-ocamba__head" }, h("p", { className: "m-eyebrow m-eyebrow--volt" }, "Who we are"), h("h2", { className: "m-h2" }, "More than a result"), h("p", { className: "mh-ocamba__desc" }, "A club built in memory and driven by purpose. Champions on the pitch, and a family off it."), h("div", { className: "mh-ocamba__nav" }, h("button", { className: "mh-ocamba__arrow", type: "button", "aria-label": "Previous", onClick: function(e) {
      var r = e.currentTarget.closest(".mh-ocamba").querySelector(".mh-pillrail");
      if (r) r.scrollBy({ left: -344, behavior: "smooth" });
    } }, "←"), h("button", { className: "mh-ocamba__arrow", type: "button", "aria-label": "Next", onClick: function(e) {
      var r = e.currentTarget.closest(".mh-ocamba").querySelector(".mh-pillrail");
      if (r) r.scrollBy({ left: 344, behavior: "smooth" });
    } }, "→"))), h("div", { className: "mh-pillrail" }, [["Champions", "League Ten winners", "Played 18, won 18, unbeaten and promoted for 26/27.", "champions", "The season"], ["Community", "A football family", "Built in south-west London, playing for each other every week.", "about", "Our story"], ["Our cause", "Sepsis awareness", "Founded in memory of Susan Anne Martin. Know the signs.", "sepsis", "Our cause"], ["Partners", "Back the badge", "Local businesses on the shirt, and there is room for yours.", "sponsors", "Partner with us"]].map(function(p, i) {
      return h("button", { key: i, className: "mh-pillar", onClick: function() {
        go(p[3]);
      } }, h("p", { className: "m-eyebrow m-eyebrow--volt" }, p[0]), h("h3", { className: "m-h3" }, p[1]), h("p", { className: "mh-pillar__body" }, p[2]), h("span", { className: "mh-pillar__cta" }, p[4] + " →"));
    }))))
  ), function() {
    var aw = window.getRecognition ? window.getRecognition("season_award").filter(function(a) {
      return a.season === (window.CURRENT_SEASON || "25/26");
    }) : [];
    return aw.length ? h("section", { className: "mh-sec", style: { paddingTop: 0 } }, h("div", { className: "m-wrap" }, h(Head, { eyebrow: (window.CURRENT_SEASON || "25/26") + " End of Season", title: "Award winners" }), h("div", { className: "mp-grid mp-g4 mh-awgrid", style: { marginTop: 18 } }, aw.map(function(a) {
      var nm = a.playerName || (a.playerId && window.playerNameByNum ? window.playerNameByNum(a.playerId) : "");
      var ph = a.imageUrl || (a.playerId && window.getPlayerPhoto ? window.getPlayerPhoto(a.playerId) : null);
      return h("button", { key: a.id, "data-tilt": "", className: "mh-awcard mp-clickable", onClick: function() {
        location.href = a.playerId ? "teams.html?player=" + a.playerId : "awards.html";
      } }, h("div", { className: "mh-awcard__photo" }, ph ? h("img", { src: ph, alt: nm, loading: "lazy" }) : h("img", { className: "mh-awcard__fallback", src: "assets/badge/sue-angels-shield.webp", alt: "" })), h("div", { className: "mh-awcard__grad" }), h("div", { className: "mh-awcard__body" }, h("p", { className: "m-eyebrow m-eyebrow--volt" }, a.title), h("h3", { className: "mh-awcard__name" }, nm), h("span", { className: "mh-awcard__cta" }, "View profile →")));
    })), h("div", { style: { marginTop: 22, textAlign: "center" } }, h("a", { className: "m-btn m-btn--ghost", href: "awards.html" }, "All awards & honours →")))) : null;
    ...
    eyebrow: "All competitions · 25/26",
    title: "The campaign",
    link: "Champions"
  }), /* @__PURE__ */ React.createElement("div", {
    className: "mh-ledger",
  ...
  }, all.l === 0 ? "Unbeaten" : `${all.w}W ${all.d}D ${all.l}L`, " · all comps")), /* @__PURE__ */ React.createElement("div", {
  ...
  }, r.pts)))))))), /* @__PURE__ */ React.createElement("section", { className: "mh-sec", style: { paddingTop: 0 } }, /* @__PURE__ */ React.createElement("div", { className: "m-wrap", style: { textAlign: "center" } }, /* @__PURE__ */ React.createElement("p", { className: "m-eyebrow m-eyebrow--volt", style: { justifyContent: "center", display: "inline-flex" } }, "Proudly backed by"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: "clamp(16px,4vw,40px)", justifyContent: "center", alignItems: "center", flexWrap: "wrap", margin: "18px 0 22px" } }, [["assets/sponsors/sporting-solutions.webp", "Sporting Solutions Ltd"], ["assets/sponsors/hodgson-roofing.webp", "Hodgson Roofing"], ["assets/sponsors/staines-rugby.png", "Staines Rugby Club"]].map(function(s) {
    return /* @__PURE__ */ React.createElement("div", { key: s[1], style: { background: "#fff", borderRadius: 12, padding: "10px 18px", display: "flex", alignItems: "center" } }, /* @__PURE__ */ React.createElement("img", { src: s[0], alt: s[1], loading: "lazy", style: { height: "clamp(30px,5vw,44px)", width: "auto", objectFit: "contain" } }));
  })), /* @__PURE__ */ React.createElement("a", { className: "m-btn m-btn--ghost", href: "sponsors.html" }, "Become a partner"))), /* @__PURE__ */ React.createElement(Join, {
    go
  }));
}
function Join({
  ...
  }, "26/27 · The next chapter"), /* @__PURE__ */ React.createElement("h2", {
    className: "m-h2",
    style: {
      marginTop: 14
    }
...
function About({
  ...
  return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(PageHero, {
    ...
    className: "m-eyebrow m-eyebrow--volt"
  }, "Off the field · Sue's story"), /* @__PURE__ */ React.createElement("h2", {
    className: "m-h2",
    style: {
      marginTop: 12
    }
...
function ManagerAnalytics({ coach }) {
  ...
  for (const r of results) {
    ...
    if (th === 0) cs++;
    if (r.kind === "penalty" && r.pens) {
      const uP = uh ? r.pens.hs : r.pens.as, tP = uh ? r.pens.as : r.pens.hs;
      if (uP > tP) w++;
      else l++;
    } else if (us > th) w++;
    else if (us === th) d++;
  ...
  const button = (active, onClick, label, key) => h("button", { key, type: "button", className: "mp-subtab " + (active ? "is-active" : ""), onClick }, label);
  ...
  for (const sk of seasonsScope) {
    ...
    for (const pp of list) {
      ...
      agg[k].apps += pp.apps || 0;
      agg[k].goals += pp.goals || 0;
      agg[k].assists += pp.assists || 0;
      agg[k].motm += pp.motm || 0;
      agg[k].cs += pp.cleanSheets || 0;
    }
  }
  ...
  const fcount = {};
  for (const r of results) {
    const dd = window.loadMatchEntry ? window.loadMatchEntry(r.id) : null;
    const f = dd && dd.formation;
  ...
  const seg = (key, val, bg) => val ? h("i", { key, style: { width: (on ? val / pl * 100 : 0) + "%", background: bg } }) : null;
  return h(
    "div",
    { className: "ma" },
    filters,
    h(
      ...
      h(
        "div",
        { className: "ma__record" },
        h(Ring, { pct: winPct / 100, value: winPct + "%", label: "Win rate", sub: w + "W " + d + "D " + l + "L", on }),
        h(
          "div",
          { className: "ma__tiles" },
    ...
    honours.length ? h(
      ...
      h("div", { className: "ma__honours" }, honours.map((ho, i) => h("div", { className: "ma__honour", key: i }, maTrophy(), h("div", null, h("b", null, ho.t), h("span", null, ho.s)))))
    ) : null,
    mostUsed.length ? h(
      "div",
      { className: "ma__block" },
      h("p", { className: "ma__title" }, "Most-used players"),
      h("div", { className: "ma__bars" }, mostUsed.map((p) => h(
        "div",
        { className: "ma__bar", key: p.num },
        h("div", { className: "ma__bar-name" }, p.name, h("span", null, p.pos)),
        h("div", { className: "ma__bar-val" }, p.apps),
        h("div", { className: "ma__bar-track" }, h("div", { className: "ma__bar-fill", style: { width: (on ? maxApps ? p.apps / maxApps * 100 : 0 : 0) + "%" } }))
      )))
    ) : null,
    topScorer || topCreator || topMotm || topKeeper ? h(
      "div",
      { className: "ma__block" },
      ...
      h(
        ...
        perfCard("Best keeper", topKeeper, topKeeper ? topKeeper.cs + " clean sheets" : "")
      )
    ) : null,
    topForm ? h(
      "div",
      ...
      h(
        ...
        h(
          "div",
          { className: "ma__form-lead" },
          h("div", { className: "ma__form-big" }, topForm[0]),
          h("span", { className: "ma__form-sub" }, topForm[1] + " of " + totForm + " games")
        ),
        h("div", { className: "ma__form-dist" }, forms.slice(0, 4).map((kv) => h(
          "div",
          { className: "ma__bar", key: kv[0] },
          h("div", { className: "ma__bar-name" }, kv[0]),
          h("div", { className: "ma__bar-val" }, kv[1]),
          h("div", { className: "ma__bar-track" }, h("div", { className: "ma__bar-fill", style: { width: (on ? totForm ? kv[1] / totForm * 100 : 0 : 0) + "%" } }))
        )))
      )
    ) : null
  );
...
function CoachModalContent({ coach }) {
  ...
  const header = h(
    "div",
    { className: "ma__hd" },
    h("img", { className: "mp-coach__ph", style: { width: "clamp(120px, 28vw, 168px)", height: "auto", aspectRatio: "3 / 4", objectFit: "cover", objectPosition: "top center", borderRadius: 18 }, src: photo || "assets/players/avatar.svg", alt: "", onError: (e) => {
  ...
  const tabs = showAnalytics ? h(
    "div",
    { className: "ma-tabs" },
    h("button", { className: `ma-tab ${slide === 0 ? "is-on" : ""}`, onClick: () => setSlide(0) }, "Profile"),
    h("button", { className: `ma-tab ${slide === 1 ? "is-on" : ""}`, onClick: () => setSlide(1) }, "Manager stats")
  ) : null;
  const profileSlide = h(
    React.Fragment,
    null,
    ...
    coach.managed ? h(
      ...
      h("div", { style: { display: "flex", flexWrap: "wrap", gap: 8, marginTop: 12 } }, coach.managed.map((m) => h("span", { className: "m-chip", key: m }, m)))
    ) : null
  );
  return h(
    "div",
...
function Team({
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("sa-roster-changed", h);
      window.removeEventListener("sa-media-changed", h);
    };
  }, []);
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        let p = null;
        if (/^\d+$/.test(raw)) p = (all || []).find((x) => x.num === parseInt(raw, 10));
        else {
          const s = raw.toLowerCase();
          p = (all || []).find((x) => saSlug(saPlayerName(x)) === s);
        }
        if (p) {
          setTab("squad");
          setProfile(p.num);
  ...
  useEffect(() => {
    if (profileFirst.current) {
      profileFirst.current = false;
      return;
    }
    try {
      if (profile != null) {
  ...
  useEffect(() => {
    if (coachFirst.current) {
      coachFirst.current = false;
      return;
    }
    try {
      if (coach) {
  ...
  return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(PageHero, {
    ...
    var _st = slim ? (tab === "squad" || tab === "past") && hasPast ? [["squad", "First team"], ["past", "Past players"]] : null : [["squad", "First team"], ["leaders", "Leaderboards"], ["coaches", "Coaches"], ["stats", "Team stats"]].concat(hasPast ? [["past", "Past players"]] : []);
    return _st ? /* @__PURE__ */ React.createElement("div", {
      className: "mp-subtabs"
  ...
  }(), tab === "squad" || tab === "stats" || tab === "past" ? /* @__PURE__ */ React.createElement("div", {
    className: "mp-subtabs"
  ...
  })))) : groups.length ? /* @__PURE__ */ React.createElement(SquadBoard, { groups, onOpen: setProfile, hideStats }) : /* @__PURE__ */ React.createElement("div", { className: "m-empty" }, /* @__PURE__ */ React.createElement("b", null, season === "26/27" ? "NO SIGNINGS YET" : "NOBODY HERE"), /* @__PURE__ */ React.createElement("span", null, season === "26/27" ? "NEW FACES FOR 26/27 LAND HERE" : "NO PLAYERS FOR THIS SEASON")) : tab === "leaders" ? /* @__PURE__ */ React.createElement(Leaderboard, null) : tab === "coaches" ? /* @__PURE__ */ React.createElement("div", {
    className: "mp-grid mp-g3"
    ...
    num: profile
  }), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 16, textAlign: "center" } }, /* @__PURE__ */ React.createElement(ShareBtn, { what: "player", label: "Share player", image: window.getPlayerPhoto ? window.getPlayerPhoto(profile) : null, story: function() {
    return window.saPlayerStorySpec ? window.saPlayerStorySpec(profile) : null;
  } }))) : null, coach ? /* @__PURE__ */ React.createElement(Modal, {
    onClose: () => setCoach(null)
  }, /* @__PURE__ */ React.createElement(CoachModalContent, {
    coach
  }), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 16, textAlign: "center" } }, /* @__PURE__ */ React.createElement(ShareBtn, { what: "coach", label: "Share coach", story: function() {
    return { title: coach.name, subtitle: coach.role || "Coach", photo: coach.photo || "assets/players/avatar.svg?v=2", footer: "suesangelsfc.co.uk" };
  } }))) : null);
}
function useLeagueTick() {
  ...
  useEffect(function() {
    ...
    return function() {
      window.removeEventListener("sa-league-changed", f);
    };
  }, []);
}
function LeagueResults() {
  ...
  var short = function(n) {
    return String(n).replace(/\s*FC\b/, "").trim();
  };
  var isUs = function(n) {
    return String(n).indexOf("Angels") > -1;
  };
  return h(
    "div",
    { className: "m-glass", style: { padding: "clamp(16px,2.4vw,24px)" } },
    ...
    h(
      "div",
      { style: { display: "flex", flexDirection: "column", gap: 3, maxHeight: 580, overflowY: "auto" } },
      list.map(function(r, i) {
        var us = isUs(r.home) || isUs(r.away);
        var score = r.wo ? h("b", { style: { color: "var(--m-ink-2)", minWidth: 44, textAlign: "center", fontSize: "0.72rem", letterSpacing: "0.04em" } }, "W/O") : h("b", { style: { color: "var(--m-volt-ink)", minWidth: 44, textAlign: "center", fontVariantNumeric: "tabular-nums" } }, r.hs + "-" + r.as);
        var homeBold = isUs(r.home) || (r.wo ? r.wo === "home" : r.hs > r.as);
        var awayBold = isUs(r.away) || (r.wo ? r.wo === "away" : r.as > r.hs);
        return h(
          "div",
          { key: i, style: { display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 12, background: us ? "rgba(214,242,58,0.07)" : "var(--m-glass-1)", borderLeft: us ? "2px solid var(--m-volt)" : "2px solid transparent" } },
          h("span", { style: { fontSize: "0.7rem", color: "var(--m-ink-3)", width: 56, flex: "0 0 auto" } }, r.date),
          h(
            "span",
            { style: { flex: 1, minWidth: 0, textAlign: "right", display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 7, overflow: "hidden" } },
          ...
          h(
            "span",
            { style: { flex: 1, minWidth: 0, display: "flex", alignItems: "center", gap: 7, overflow: "hidden" } },
...
function LeagueStatsTable() {
  ...
  return h(
    "div",
    { className: "m-glass", style: { padding: "8px 8px 4px", overflowX: "auto" } },
    h(
      "div",
      { style: { padding: "14px 14px 8px", display: "flex", flexWrap: "wrap", gap: 10, alignItems: "flex-end", justifyContent: "space-between" } },
      h(
        "div",
        null,
        h("span", { className: "m-eyebrow m-eyebrow--volt" }, "Southern Sunday League · 25/26"),
      ...
      h(
        "div",
        { className: "mp-subtabs", style: { margin: 0 } },
        h("button", { className: "mp-subtab " + (mode === "all" ? "is-active" : ""), onClick: function() {
          setMode("all");
        } }, "All comps"),
        h("button", { className: "mp-subtab " + (mode === "league" ? "is-active" : ""), onClick: function() {
          setMode("league");
        } }, "League only")
      )
    ),
    h(
      ...
      h("tbody", null, rows.map(function(s) {
        return h(
          "tr",
          { key: s.pos, className: s.us ? "is-us" : "" },
...
function LeagueTable() {
  ...
  }, "League Ten · 25/26"), /* @__PURE__ */ React.createElement("button", {
    className: `mp-subtab ${season === "26/27" ? "is-active" : ""}`,
    onClick: () => setSeason("26/27")
  }, "League Eight · 26/27")), season === "25/26" ? /* @__PURE__ */ React.createElement("div", {
    className: "m-glass",
    style: {
      padding: "8px 8px 4px",
      overflowX: "auto"
  ...
  }, "Vacancy, to be confirmed"))))))), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 18, textAlign: "center" } }, /* @__PURE__ */ React.createElement(ShareBtn, { what: "table", label: "Share table", title: "League Ten table · Sue's Angels FC", url: "table.html", story: function() {
    return { kind: "table", eyebrow: "Southern Sunday League · 25/26", title: "League Ten Table", rows: (rows || []).map(function(r) {
      return { pos: r.p, club: String(r.c || "").replace(" FC", ""), pl: r.pl, w: r.w, d: r.d, l: r.l, gd: r.gd, pts: r.pts, us: r.us };
    }), footer: "suesangelsfc.co.uk" };
  } })));
}
function Schedule({
...
function Media({
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("sa-articles-changed", onCh);
      window.removeEventListener("sa-media-changed", onCh);
    };
  }, []);
  const Badge = window.TeamBadge;
  ...
  const pd = (s) => {
    const m = /(\d{1,2})\s+(\w{3})\s+(\d{2,4})/.exec(s || "");
    return m ? new Date(+m[3] < 100 ? 2e3 + +m[3] : +m[3], MON2[m[2]] || 0, +m[1]).getTime() : 0;
  };
  const trunc = (s, n) => {
  ...
  const reports = (window.getDerivedResults ? window.getDerivedResults() : []).filter((r) => r.kind !== "walkover" && r.hs != null).slice(0, 12).map((r) => ({ type: "report", id: r.id, r, date: r.date, sort: pd(r.date) }));
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!aid && !rid && !albid && !vidid) {
        mediaEntry.current = true;
        return;
      }
      if (aid) {
        ...
        const al = (window.getGalleryAlbums ? window.getGalleryAlbums() : []).find((x) => x.id === albid);
        if (al) {
          mediaEntry.current = true;
          setTab("gallery");
          setAlbum(al);
        ...
        const v = (window.getClubVideos ? window.getClubVideos() : []).find((x) => x.id === vidid);
        if (v) {
          mediaEntry.current = true;
          setTab("videos");
          setVid(v);
  ...
  useEffect(() => {
    if (articleFirst.current) {
      articleFirst.current = false;
      return;
    }
    try {
      if (article) {
        window.history.replaceState(null, "", "media.html?article=" + article.id);
        document.title = (article.title || "Article") + " · Sue's Angels FC";
      } else {
        window.history.replaceState(null, "", "media.html");
        document.title = "Media · Sue's Angels FC";
      }
    } catch (e) {
    }
  }, [article]);
  ...
  useEffect(() => {
    if (reportFirst.current) {
      reportFirst.current = false;
      return;
    }
    try {
      if (report) {
        window.history.replaceState(null, "", "media.html?report=" + report.id);
        document.title = (report.home ? report.home.replace(" FC", "") + " v " + report.away.replace(" FC", "") : "Match report") + " · Sue's Angels FC";
      } else {
        window.history.replaceState(null, "", "media.html");
        document.title = "Media · Sue's Angels FC";
      }
  ...
  useEffect(() => {
    if (albumFirst.current) {
      albumFirst.current = false;
      return;
    }
    try {
      if (album) {
        window.history.replaceState(null, "", "media.html?album=" + album.id);
        document.title = (album.title || "Gallery") + " · Sue's Angels FC";
      } else {
        window.history.replaceState(null, "", "media.html");
        document.title = "Media · Sue's Angels FC";
      }
  ...
  useEffect(() => {
    if (vidFirst.current) {
      vidFirst.current = false;
      return;
    }
    try {
      if (vid) {
        window.history.replaceState(null, "", "media.html?video=" + vid.id);
        document.title = (vid.title || "Video") + " · Sue's Angels FC";
      } else {
        window.history.replaceState(null, "", "media.html");
        document.title = "Media · Sue's Angels FC";
      }
  ...
  const galleryCard = (it, i) => {
    ...
    const coverEl = matchday ? h("div", { className: "mp-news__cover", style: { gap: 10 } }, it.photographer ? h("span", { className: "mp-news__ft", style: { color: "rgb(214,242,58)", fontSize: "7px" } }, "PICTURES TAKEN BY " + String(it.photographer).toUpperCase()) : null, h("span", { className: "mp-news__ft" }, "MATCHDAY"), h("div", { className: "mp-news__sc" }, h("img", { className: "mp-news__gbadge", src: it.homeBadge || "assets/badge/sue-angels-shield.webp", alt: "" }), h("span", { className: "mp-news__gvs" }, "VS"), it.awayBadge ? h("img", { className: "mp-news__gbadge", src: it.awayBadge, alt: "" }) : null)) : h("div", { className: "mp-news__cover" }, cover ? h("img", { src: cover, alt: "" }) : h("img", { className: "gh", src: "assets/badge/sue-angels-shield.webp", alt: "" }));
    return h("button", { className: "mp-news mp-clickable", key: i, onClick: () => {
      setAlbum(it);
      setAi(0);
    } }, coverEl, it.category ? h("div", { className: "mp-news__body" }, h("span", { className: "m-chip m-chip--volt mp-news__tag" }, it.category)) : null);
  };
  ...
  return h(React.Fragment, null, h(PageHero, { eyebrow: mHero.eb, title: h(React.Fragment, null, mHero.a, h("em", null, mHero.b), "."), sub: mHero.sub }), h("section", { className: "mp-sec" }, h("div", { className: "m-wrap" }, slim ? null : h("div", { className: "mp-subtabs" }, [["live", "Live"], ["news", "News"], ["gallery", "Gallery"], ["videos", "Videos"]].map(([k, l]) => h("button", { key: k, className: `mp-subtab ${tab === k ? "is-active" : ""}`, onClick: () => setTab(k) }, l))), tab === "news" ? h(React.Fragment, null, catTabs, newsBody) : tab === "videos" ? videosBody : h(React.Fragment, null, gcatTabs, galleryBody))), report ? h(Modal, { onClose: () => setReport(null) }, h("div", { className: "m-glass m-modal__sponsor" }, h("p", { className: "m-eyebrow m-eyebrow--volt" }, report.competition || "League Ten", " · ", report.date), h("h2", { className: "m-h2", style: { marginTop: 10 } }, report.home.replace(" FC", ""), " ", report.hs, "-", report.as, " ", report.away.replace(" FC", "")), h("div", { className: "m-prose" }, function() {
    ...
    t = t && String(t).trim();
    if (!t) return h("p", null, "Match report to follow, watch this space.");
    return t.split(/\n+/).map(function(p, i) {
      return h("p", { key: i }, p);
    });
  }()), h("div", { style: { marginTop: 18, textAlign: "center" } }, h(ShareBtn, { what: "report", label: "Share report", story: function() {
    return window.saReportStorySpec ? window.saReportStorySpec(report) : null;
  } })))) : null, article ? h(Modal, { onClose: () => setArticle(null) }, h("div", { className: "m-glass m-modal__sponsor" }, article.cover ? h("img", { src: article.cover, alt: "", style: { width: "100%", maxHeight: 340, objectFit: "cover", borderRadius: 14, marginBottom: 18 } }) : null, h("p", { className: "m-eyebrow m-eyebrow--volt" }, article.cat, " · ", article.date), h("h2", { className: "m-h2", style: { marginTop: 10 } }, article.title), h("div", { className: "m-prose", style: { marginTop: 16 } }, String(article.body || "").split(/\n+/).filter(Boolean).map((p, i) => h("p", { key: i }, p))), h("div", { style: { marginTop: 18, textAlign: "center" } }, h(ShareBtn, { what: "article", label: "Share post", image: article.cover || null, story: function() {
    return { title: article.title, subtitle: article.cat || "News", photo: article.cover || null, footer: "suesangelsfc.co.uk" };
  } })))) : null, album ? function() {
    var ph = window.galleryPhotos ? window.galleryPhotos(album) : album.photos || [];
    if (!ph.length) return null;
    var idx = (ai % ph.length + ph.length) % ph.length;
    var tags = album.photoTags && album.photoTags[idx] || [];
    return h("div", { className: "m-zoom m-albumbox", onClick: () => setAlbum(null) }, h("button", { className: "m-modal__close", onClick: () => setAlbum(null) }, "✕"), h("div", { style: { position: "absolute", top: 16, left: 16, zIndex: 6 }, onClick: function(e) {
      e.stopPropagation();
    } }, h(ShareBtn, { what: "album", label: "Share", url: "media.html?album=" + (album.id || ""), story: function() {
      return { title: album.title || "Matchday gallery", subtitle: album.category || "Gallery", photo: album.cover || ph[0] || null, footer: "suesangelsfc.co.uk" };
    } })), ph.length > 1 ? h("button", { className: "m-albumbox__nav m-albumbox__nav--prev", onClick: (e) => {
      e.stopPropagation();
      setAi(idx - 1);
    } }, "‹") : null, h("figure", { className: "m-albumbox__fig", onClick: (e) => e.stopPropagation() }, h("img", { src: ph[idx], alt: "" }), tags.length ? h("figcaption", { className: "m-albumbox__tags" }, tags.join(" · ")) : null, h("span", { className: "m-albumbox__count" }, idx + 1 + " / " + ph.length)), ph.length > 1 ? h("button", { className: "m-albumbox__nav m-albumbox__nav--next", onClick: (e) => {
      e.stopPropagation();
      setAi(idx + 1);
    } }, "›") : null);
  }() : null, vid ? h("div", { className: "m-zoom", onClick: () => setVid(null) }, h("button", { className: "m-modal__close", onClick: () => setVid(null) }, "✕"), h("div", { style: { position: "absolute", top: 16, left: 16, zIndex: 6 }, onClick: function(e) {
    e.stopPropagation();
  } }, h(ShareBtn, { what: "video", label: "Share", url: "media.html?video=" + (vid.id || ""), story: function() {
    var u = vid.url || "";
    var m = u.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/))([\w-]+)/);
    return { title: vid.title || "Club video", subtitle: "Video", photo: m ? "https://img.youtube.com/vi/" + m[1] + "/hqdefault.jpg" : null, footer: "suesangelsfc.co.uk" };
  } })), function() {
    var u = vid.url || "";
    var m = u.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/))([\w-]+)/);
    if (m) return h("iframe", { src: "https://www.youtube.com/embed/" + m[1] + "?autoplay=1", style: { width: "min(92vw, 960px)", aspectRatio: "16 / 9", border: 0, borderRadius: 14 }, allow: "autoplay; fullscreen", allowFullScreen: true });
    return h("video", { src: u, controls: true, autoPlay: true, style: { width: "min(92vw, 960px)", maxHeight: "82vh", borderRadius: 14, background: "#000" } });
  }()) : null, zoom ? h("div", { className: "m-zoom", onClick: () => setZoom(null) }, h("button", { className: "m-modal__close", onClick: () => setZoom(null) }, "✕"), h("img", { src: zoom, alt: "" })) : null);
}
function maGenCover(spec) {
  ...
  return h(
    "div",
    { className: "mp-news__cover mp-news__cover--score" },
    spec.top ? h("span", { className: "mp-news__ft" }, spec.top) : null,
    h(
      "div",
      { className: "mp-news__sc" },
    ...
    spec.bottom || resLabel ? h(
      "div",
      { className: "mp-news__cfoot" },
      spec.bottom ? h("span", { className: "mp-news__comp" }, spec.bottom) : null,
      resLabel ? h("span", { className: "mp-news__res mp-news__res--" + spec.result }, resLabel) : null
    ) : null
  );
}
function maHeart() {
...
function DonateBlock() {
  ...
  React.useEffect(function() {
    ...
    window.addEventListener("sa-media-changed", f);
    return function() {
      window.removeEventListener("sa-media-changed", f);
    };
  }, []);
  ...
  var clubBtn = clubUrl ? h("a", { className: "mp-donate2__btn", href: clubUrl, target: "_blank", rel: "noopener" }, "Donate" + (typeof amount === "number" ? " £" + amount : "") + " securely", maArrow()) : h("span", { className: "mp-donate2__btn is-disabled", title: "Card payments go live once the club's Stripe link is connected." }, "Donations opening soon");
  return h(
    "div",
    { className: "mp-donate2" },
    h(
      "div",
      { className: "mp-donate2__card mp-donate2__card--club" },
      ...
      h("div", { className: "mp-donate2__amts" }, amounts.map(function(a) {
        return h("button", { type: "button", key: a, className: "mp-donate2__amt " + (amount === a ? "is-on" : ""), onClick: function() {
          setAmount(a);
        } }, a === "Custom" ? "Custom" : "£" + a);
      })),
      clubBtn,
    ...
    h(
      "a",
      { className: "mp-donate2__card mp-donate2__card--cause", href: sepsisUrl, target: "_blank", rel: "noopener" },
      h("div", { className: "mp-donate2__ic" }, maHeart()),
...
function SupporterSignup(props) {
  ...
  function submit(e) {
    ...
    if (!consent) {
      setStatus("err");
      setMsg("Please tick the box so we have your permission to email you.");
      return;
    }
    setStatus("sending");
    setMsg("");
    (window.saAddSupporter ? window.saAddSupporter(email, nm, source) : Promise.resolve({ ok: false })).then(function(r) {
      if (r && r.ok) {
        setStatus("ok");
        setMsg(r.duplicate ? "You’re already on the list — thank you." : "Thank you. You’re on the list.");
      } else {
        setStatus("err");
        setMsg("Sorry, something went wrong. Please try again shortly.");
      }
    });
  }
  var emailField = function(cls) {
    return h("input", { type: "email", value: email, required: true, autoComplete: "email", placeholder: "you@email.com", onChange: function(e) {
  ...
  var checkbox = function(size) {
    return h(
      "label",
      { style: { display: "flex", gap: size === "sm" ? 8 : 10, alignItems: "flex-start", fontSize: size === "sm" ? "0.74rem" : "0.84rem", color: size === "sm" ? "var(--m-ink-3)" : "var(--m-ink-2)", cursor: "pointer", lineHeight: 1.5 } },
      h("input", { type: "checkbox", checked: consent, onChange: function(e) {
        setConsent(e.target.checked);
      }, style: { marginTop: 3, flex: "0 0 auto", accentColor: "var(--m-volt)" } }),
  ...
  if (compact) {
    ...
    return h(
      "form",
      { onSubmit: submit, style: { display: "grid", gap: 10, maxWidth: 320 } },
      h("input", { type: "email", value: email, required: true, autoComplete: "email", placeholder: "Your email", onChange: function(e) {
        setEmail(e.target.value);
      }, style: { width: "100%", padding: "11px 13px", borderRadius: 10, border: "1px solid var(--m-edge)", background: "var(--m-glass-1)", color: "var(--m-ink-1)", font: "500 15px var(--m-sans)" } }),
      checkbox("sm"),
  ...
  if (status === "ok") return h(
    "div",
    { className: "m-glass", style: { padding: "clamp(22px,3vw,30px)", textAlign: "center" } },
  ...
  return h(
    "form",
    { className: "m-glass mp-form", onSubmit: submit },
    h(
      "div",
      { className: "mp-frow" },
      h("label", { className: "mp-field" }, h("span", null, "Name (optional)"), h("input", { type: "text", value: nm, autoComplete: "name", placeholder: "Your name", onChange: function(e) {
...
function SponsorPackGate() {
  ...
  function submit(e) {
    ...
    setStatus("sending");
    (window.saAddEnquiry ? window.saAddEnquiry({ type: "Sponsorship pack request", email, message: "Downloaded the sponsorship pack.", source: "sponsor-pack" }) : Promise.resolve({ ok: false })).then(function() {
      setStatus("done");
      dl();
    });
  }
  return h(
    React.Fragment,
    null,
    h("button", { className: "m-btn m-btn--ghost", onClick: function() {
      setOpen(true);
    } }, "Download the pack"),
    open ? h(Modal, { onClose: function() {
      setOpen(false);
    } }, h(
      "div",
      { className: "m-glass", style: { padding: "clamp(22px,3vw,30px)", maxWidth: 440 } },
      status === "done" ? h("div", { style: { textAlign: "center" } }, h("p", { className: "m-eyebrow m-eyebrow--volt", style: { justifyContent: "center", display: "inline-flex" } }, "On its way"), h("h3", { className: "m-h3", style: { marginTop: 10 } }, "Thanks — your download has started."), h("p", { style: { color: "var(--m-ink-3)", marginTop: 8, fontSize: "0.9rem" } }, "If it didn’t open, ", h("a", { href: PDF, target: "_blank", rel: "noopener", style: { color: "var(--m-volt-ink)" } }, "tap here"), ". We’ll be in touch about partnering.")) : h(
        "form",
        { onSubmit: submit },
        ...
        h("input", { type: "email", required: true, value: email, placeholder: "you@company.com", autoComplete: "email", onChange: function(e) {
          setEmail(e.target.value);
        }, style: { width: "100%", padding: "12px 14px", borderRadius: 10, border: "1px solid var(--m-edge)", background: "var(--m-glass-1)", color: "var(--m-ink-1)", font: "500 15px var(--m-sans)" } }),
...
function Sponsors({
  ...
  return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(PageHero, {
    ...
    className: "m-h3"
  }, t), /* @__PURE__ */ React.createElement("p", null, c)))))), h("section", { className: "mp-sec", style: { paddingTop: 0 } }, h(
    "div",
    { className: "m-wrap" },
    ...
    className: "m-eyebrow m-eyebrow--volt"
  }, "Official partner · since ", detail.since), /* @__PURE__ */ React.createElement("h2", {
    className: "m-h2",
    style: {
      marginTop: 8,
      fontSize: "2rem"
...
function saSendEnquiry(form, label) {
  ...
  var capture = window.saAddEnquiry ? window.saAddEnquiry({ type: label, name, email, phone: extra, message, source: "enquiry" }) : Promise.resolve({ ok: false });
...
function Contact() {
  ...
  }, "susangelsfc@gmail.com", /* @__PURE__ */ React.createElement("br", null), "@suesangelsfc · Instagram"))), /* @__PURE__ */ React.createElement("form", {
    className: "m-glass mp-form",
    onSubmit: (e) => {
      e.preventDefault();
      saSendEnquiry(e.target, routes.find((r) => r[0] === route)[1]).then(function(r) {
        setSent(r && r.ok ? "ok" : "mail");
      });
    }
...
function Sepsis({ go }) {
  ...
  const help = [
    ["Know and share the signs", "The signs above can save a life. Take a moment to learn them, and share them with the people you love.", "Share this page", () => {
      try {
        if (navigator.share) navigator.share({ title: "Know the signs of sepsis", url: location.href });
  ...
  return h(
    ...
    h("section", { className: "mp-sec", style: { paddingTop: 0 } }, h(
      "div",
      { className: "m-wrap" },
      h(
        "div",
        { className: "m-glass", style: { padding: "clamp(24px,3vw,44px)" } },
        ...
        h(
          "div",
          { className: "m-prose", style: { marginTop: 18 } },
    ...
    h("section", { className: "mp-sec", style: { paddingTop: 0 } }, h(
      ...
      h(
        "div",
        { className: "m-prose", style: { maxWidth: "70ch" } },
    ...
    h("section", { id: "signs", className: "mp-sec", style: { paddingTop: 0 } }, h(
      "div",
      { className: "m-wrap" },
      ...
      h("div", { className: "mp-grid mp-g3" }, signs.map((s, i) => h(
        "div",
        { key: i, className: "m-glass", style: { padding: "20px 22px" } },
        h("h3", { className: "m-h3", style: { fontSize: "1.1rem" } }, s[0]),
        h("p", { style: { marginTop: 8, color: "var(--m-ink-2)", fontSize: "0.92rem" } }, s[1])
      ))),
      h(
        "div",
        { className: "m-glass", style: { padding: "clamp(20px,3vw,30px)", marginTop: 18 } },
      ...
      h(
        "div",
        { className: "m-glass", style: { padding: "clamp(20px,3vw,32px)", marginTop: 18, borderColor: "var(--m-volt)" } },
    ...
    h("section", { className: "mp-sec", style: { paddingTop: 0 } }, h(
      ...
      h("div", { className: "mp-grid mp-g3" }, help.map((c, i) => h(
        "div",
        { key: i, className: "m-glass", style: { padding: "clamp(22px,3vw,30px)", display: "flex", flexDirection: "column", gap: 12 } },
        h("h3", { className: "m-h3" }, c[0]),
        h("p", { style: { color: "var(--m-ink-2)", flex: 1 } }, c[1]),
        typeof c[3] === "string" ? h("a", { className: "m-btn m-btn--ghost", href: c[3], target: "_blank", rel: "noopener", style: { alignSelf: "flex-start" } }, c[2]) : h("button", { className: "m-btn m-btn--ghost", onClick: c[3], style: { alignSelf: "flex-start" } }, c[2])
      )))
    )),
    h("section", { id: "give", className: "mp-sec", style: { paddingTop: 0 } }, h(
      "div",
      { className: "m-wrap" },
    ...
    h("section", { id: "stay", className: "mp-sec", style: { paddingTop: 0 } }, h(
      "div",
      { className: "m-wrap" },
    ...
    h("section", { className: "mp-sec", style: { paddingTop: 0 } }, h(
      ...
      h(
        "div",
        { className: "m-glass", style: { padding: "clamp(28px,4vw,52px)", textAlign: "center" } },
...
function Records({ go }) {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("sa-recognition-changed", on);
      window.removeEventListener("sa-match-changed", on);
    };
  }, []);
  ...
  const oi = (r) => {
    const i = ORDER.indexOf(r.recordKey);
    return i < 0 ? 99 : i;
  };
  ...
  const card = (r, i) => {
    ...
    if (r.playerId) props.onClick = () => {
      window.location.href = "teams.html?player=" + r.playerId;
    };
    return h(r.playerId ? "button" : "div", props, inner);
  };
  const trophyCard = (t, i) => h(
    "div",
    { key: t.id, className: "rec-trophy m-glass rec-card--in", style: { animationDelay: i * 0.08 + "s" } },
    h("span", { className: "rec-trophy__ic" }, I[t.icon || "trophy"]),
    h(
      "div",
      { className: "rec-trophy__body" },
  ...
  return h(
    ...
    trophies.length ? h("section", { className: "mp-sec" }, h(
      "div",
      { className: "m-wrap" },
      h(Head, { eyebrow: "Honours", title: "Trophies won" }),
      h("div", { className: "rec-trophies" }, trophies.map(trophyCard))
    )) : null,
    h("section", { className: "mp-sec", style: trophies.length ? { paddingTop: 0 } : null }, h(
      "div",
      { className: "m-wrap" },
    ...
    lead ? h("section", { className: "mp-sec", style: { paddingTop: 0 } }, h(
      "div",
      { className: "m-wrap" },
      h(Head, { eyebrow: lead.season + " season", title: "Leadership group" }),
      h(
        "div",
        { className: "m-glass rec-lead" },
        h("p", { className: "rec-lead__note" }, lead.note),
        h("div", { className: "rec-lead__grid" }, [["Club captain", lead.clubCaptainName, lead.clubCaptainPlayerId, true], ["Vice-captain", lead.viceCaptainName, lead.viceCaptainPlayerId, false], ["Third-choice captain", lead.thirdChoiceCaptainName, lead.thirdChoiceCaptainPlayerId, false]].filter((x) => x[1]).map((x, i) => h(
          "div",
          { key: i, className: "rec-lead__role" + (x[3] ? " is-captain" : "") },
          h("span", { className: "rec-lead__label" }, x[0]),
          x[2] ? h("button", { className: "rec-lead__name mp-clickable", onClick: () => {
            window.location.href = "teams.html?player=" + x[2];
          } }, x[1]) : h("span", { className: "rec-lead__name" }, x[1])
        )))
      )
    )) : null
  );
...
function Awards({ go }) {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("sa-recognition-changed", on);
      window.removeEventListener("sa-photo-changed", on);
    };
  }, []);
  ...
  const monKey = (p) => {
    const yrs = String(p.season || "").split("/");
    const idx = monIdx(p.month);
    const fh = idx >= 0 && idx <= 4;
    return ((fh ? yrs[0] : yrs[1]) || "") + ("0" + (idx + 1)).slice(-2);
  };
  const orderKey = (p) => {
    const o = parseInt(p.order, 10);
    return o > 0 ? "A" + ("00000" + o).slice(-6) : "B" + monKey(p);
  };
  ...
  const chips = (r) => {
    ...
    const s = [];
    if (apps) s.push([apps, "Apps"]);
    if (goals) s.push([goals, "Goals"]);
    if (assists) s.push([assists, "Assists"]);
    if (cs) s.push([cs, "Clean sheets"]);
    if (motm) s.push([motm, "MOTM"]);
    return s;
  };
  const motmList = window.getMotmList ? window.getMotmList(season) : [];
  const heroCard = (r) => h(
    "div",
    { className: "aw-potm m-glass", key: r.id },
    ...
    h(
      ...
      h(
        "div",
        { className: "aw-potm__actions" },
        r.playerId ? h("button", { className: "m-btn m-btn--ghost", onClick: () => {
          window.location.href = "teams.html?player=" + r.playerId;
        } }, "View profile") : null,
        h(ShareBtn, { what: "potm", label: "Share", url: "awards.html", story: function() {
          return { title: nameFor(r), subtitle: "Player of the Month · " + (r.month || ""), eyebrow: (r.season || season) + " · Player of the Month", photo: photoFor(r), face: !!photoFor(r), footer: "suesangelsfc.co.uk" };
        } })
      )
    )
  );
  const awardCard = (a) => h(
    a.playerId ? "button" : "div",
    { key: a.id, className: "aw-card m-glass" + (a.playerId ? " mp-clickable" : ""), onClick: a.playerId ? () => {
      window.location.href = "teams.html?player=" + a.playerId;
    } : void 0 },
    media(photoFor(a), "aw-card__media"),
    h(
      "div",
      { className: "aw-card__body" },
  ...
  return h(
    ...
    h("section", { className: "mp-sec" }, h(
      "div",
      { className: "m-wrap" },
      seasonTabs.length ? h("div", { className: "aw-tabs rec-seasons", style: { marginBottom: 18 } }, seasonTabs.map((s) => h("button", { key: s, type: "button", className: "aw-tab" + (season === s ? " is-active" : ""), onClick: () => {
        setSeason(s);
        setSelId(null);
      } }, h("span", { className: "aw-tab__m" }, s)))) : null,
      ...
      latest ? h(
        ...
        heroCard(sel)
      ) : empty("Player of the Month coming soon", "Our first monthly winner will be revealed here.")
    )),
    motmList.length ? h("section", { className: "mp-sec", style: { paddingTop: 0 } }, h(
      "div",
      { className: "m-wrap" },
      h(Head, { eyebrow: "Every match", title: "Man of the Match" }),
      h("div", { className: "aw-motm", style: { maxHeight: 288, overflowY: "auto", paddingRight: 6, scrollbarWidth: "thin" } }, motmList.map((m, i) => h(
        m.playerId ? "button" : "div",
        { key: i, className: "aw-motm__row" + (m.playerId ? " mp-clickable" : ""), onClick: m.playerId ? () => {
          window.location.href = "teams.html?player=" + m.playerId;
        } : void 0 },
        h("span", { className: "aw-motm__match" }, m.opp, " ", h("b", null, m.score)),
        h("span", { className: "aw-motm__date" }, m.date),
        h("span", { className: "aw-motm__winner" }, m.playerName)
      )))
    )) : null,
    h("section", { className: "mp-sec", style: { paddingTop: 0 } }, h(
      "div",
...
function SiteFooter() {
  ...
  return h("footer", { className: "mh-footer" }, h(
    "div",
    { className: "m-wrap" },
    h(
      "div",
      { className: "mh-footer__row", style: { alignItems: "flex-start" } },
      h(
        "div",
        null,
      ...
      h("div", { style: { display: "flex", gap: 10, flexWrap: "wrap" } }, social.map(function(s) {
        var ic = s[0] === "Instagram" ? h("svg", { width: 22, height: 22, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": "true" }, h("rect", { x: 2, y: 2, width: 20, height: 20, rx: 5, ry: 5 }), h("path", { d: "M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" }), h("line", { x1: 17.5, y1: 6.5, x2: 17.51, y2: 6.5 })) : h("svg", { width: 21, height: 21, viewBox: "0 0 24 24", fill: "currentColor", "aria-hidden": "true" }, h("path", { d: "M12.53.02C13.84 0 15.14.01 16.44 0c.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" }));
    ...
    h(
      "div",
      { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 150px), 1fr))", gap: 24, marginTop: 32 } },
      groups.map(function(g) {
        return h(
          ...
          g[1].map(function(it) {
            return h("a", { key: it[0], href: HREF[it[0]], style: linkStyle }, it[1]);
          })
        );
      })
    ),
    h(
      "div",
      { style: { marginTop: 30, paddingTop: 24, borderTop: "1px solid var(--m-edge)", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 270px), 1fr))", gap: 22, alignItems: "start" } },
      h(
        "div",
        null,
    ...
    h(
      "div",
      { style: { marginTop: 26, paddingTop: 18, borderTop: "1px solid var(--m-edge)", display: "flex", flexWrap: "wrap", gap: "6px 18px", alignItems: "center" } },
      h("span", { style: { font: "700 0.64rem var(--m-sans)", letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--m-ink-3)" } }, "Also"),
      supporting.map(function(it) {
        return h("a", { key: it[0], href: it[0], style: { color: "var(--m-ink-3)", fontSize: "0.72rem", letterSpacing: "0.06em", textTransform: "uppercase", textDecoration: "none" } }, it[1]);
      })
    ),
    h("div", { className: "sa-vh" }, h("small", null, "Sunday-league football at The Reeves, Hanworth · serving Kingston, Sunbury, Staines and south-west London · founded 2025 in memory of Susan Anne Martin · League Ten champions 25/26 · supporting sepsis awareness"))
  ));
}
function BackToTop() {
...
function SquadBoard(props) {
  ...
  function flip(e) {
    e.preventDefault();
    e.stopPropagation();
    var c = e.currentTarget.closest(".mp-player");
  ...
  function card(p) {
    ...
    var frontStats = p.gk ? [[p.apps, "Apps"], [p.cleanSheets, "CS"], [p.motm, "MOTM"]] : [[p.apps, "Apps"], [p.goals, "Goals"], [p.assists, "Assists"]];
    var backStats = p.gk ? [[p.apps, "Apps"], [p.cleanSheets, "Clean sheets"], [p.started != null ? p.started : p.apps, "Starts"], [p.motm, "MOTM"]] : [[p.goals, "Goals"], [p.assists, "Assists"], [ga, "G+A"], [p.apps, "Apps"], [p.started != null ? p.started : p.apps, "Starts"], [p.motm, "MOTM"]];
    return h(
      "div",
      { className: "mp-player mp-player--flip", key: p.num, "data-num": p.num },
      h(
        "div",
        { className: "mp-player__inner" },
        h(
          "button",
          { className: "mp-player__face mp-player__front mp-clickable", onClick: function() {
            onOpen && onOpen(p.num);
          } },
          h(
            "div",
          ...
          h(
            ...
            h("div", { className: "mp-player__stats" }, frontStats.map(function(s, k) {
              return h("div", { key: k }, h("b", null, hideStats ? "–" : h(CountNum, { value: s[0] })), h("span", null, s[1]));
            }))
          )
        ),
        h(
          "div",
          { className: "mp-player__face mp-player__back" },
          h(
            "div",
            { className: "mp-pb__head" },
            h("div", { className: "mp-pb__id" }, h("b", null, p.first + " " + p.last), h("span", null, p.gk ? "Goalkeeper" : pos))
          ),
          h("div", { className: "mp-pb__grid" }, backStats.map(function(s, k) {
            return h("div", { className: "mp-pb__stat", key: k }, h("b", null, hideStats ? "–" : h(CountNum, { value: s[0] != null ? s[0] : 0 })), h("span", null, s[1]));
          })),
          h("button", { className: "m-btn m-btn--volt mp-pb__cta", onClick: function() {
            onOpen && onOpen(p.num);
          } }, "Full profile")
        )
      ),
      h(
        "button",
        { className: "mp-player__flipbtn", onClick: flip, type: "button", "aria-label": "Flip card for stats", title: "Flip for stats" },
        h(
          "svg",
  ...
  return h(
    "div",
    { className: "mp-squadboard" },
    h("div", { className: "mp-posselect" }, groups.map(function(g) {
      return h(
        "button",
        { key: g[0], type: "button", className: "mp-posselect__btn " + (g[0] === active ? "is-active" : ""), onClick: function() {
          setActive(g[0]);
        } },
        h("span", { className: "mp-posselect__lbl" }, g[0]),
        h("i", { className: "mp-posselect__n" }, g[1].length)
      );
    })),
...
function SparkChart(props) {
  ...
  var lines = isGK ? [{ k: "Clean sheets", d: CS, c: "var(--m-volt)", glow: true }] : [{ k: "G + A", d: GA, c: "var(--m-volt)", glow: true }, { k: "Goals", d: G, c: "var(--m-ink-1)" }, { k: "Assists", d: A, c: "#22D3EE" }];
  ...
  var xx = function(i) {
    return L0 + (n <= 1 ? (W - L0 - R) / 2 : i / (n - 1) * (W - L0 - R));
  };
  var yy = function(v) {
    return H - B - v / maxY * (H - B - T);
  };
  var path = function(d) {
    return d.map(function(v, i) {
      return (i ? "L" : "M") + xx(i).toFixed(1) + " " + yy(v).toFixed(1);
    }).join(" ");
  };
  var yt = [0, Math.round(maxY / 2), maxY].filter(function(v, i, a) {
    return a.indexOf(v) === i;
  });
  var xt = n <= 1 ? [0] : [0, Math.floor((n - 1) / 2), n - 1];
  return h(
    "div",
    { className: "m-sparkwrap" },
    h(
      "svg",
      { className: "m-spark m-spark--axes", viewBox: "0 0 " + W + " " + H },
      h("defs", null, h(
        "filter",
        { id: "m-sparkglow", x: "-20%", y: "-50%", width: "140%", height: "200%" },
        h("feGaussianBlur", { stdDeviation: "2.6", result: "b" }),
        h("feMerge", null, h("feMergeNode", { in: "b" }), h("feMergeNode", { in: "SourceGraphic" }))
      )),
      yt.map(function(t, i) {
        return h("g", { key: "y" + i }, h("line", { x1: L0, y1: yy(t), x2: W - R, y2: yy(t), className: "m-spark__grid" }), h("text", { x: L0 - 9, y: yy(t) + 3.5, textAnchor: "end", className: "m-spark__tick" }, t));
      }),
      xt.map(function(xi, i) {
        return h("text", { key: "x" + i, x: xx(xi), y: H - 11, textAnchor: i === 0 ? "start" : i === xt.length - 1 ? "end" : "middle", className: "m-spark__tick" }, "Game " + (xi + 1));
      }),
      lines.map(function(L, li) {
        return h("path", { key: "ln" + li, className: "m-spark__animline", d: path(L.d), fill: "none", stroke: L.c, strokeWidth: L.glow ? 3 : 2, strokeLinejoin: "round", strokeLinecap: "round", filter: L.glow ? "url(#m-sparkglow)" : null, opacity: L.glow ? 1 : 0.9, pathLength: 1, style: { animationDelay: li * 0.12 + "s" } });
      }),
      lines.map(function(L, li) {
        var lv = L.d[L.d.length - 1] || 0;
        return h("circle", { key: "lp" + li, className: "m-spark__dot", cx: xx(n - 1), cy: yy(lv), r: L.glow ? 4.5 : 3, fill: L.c, stroke: "var(--m-bg)", strokeWidth: 1, style: { animationDelay: 1.05 + li * 0.12 + "s" } });
      })
    ),
    h("div", { className: "m-sparklegend" }, lines.map(function(L, li) {
      return h("span", { key: "lg" + li, className: "m-sparklegend__i" }, h("i", { style: { background: L.c } }), L.k + " · " + (L.d[L.d.length - 1] || 0));
    }))
  );
}
...
function StatCounters(props) {
  ...
  var rows = squad.map(function(s) {
    ...
    var feat = lg.length, w = lg.filter(function(m) {
      return m.res === "w";
    }).length;
    var cs = lg.filter(function(m) {
      return m.csPart;
    }).length;
    var conc = lg.reduce(function(a, m) {
      return a + (m.conceded || 0);
    }, 0);
    return { num: s.num, apps: s.apps || 0, started: s.started || 0, goals: s.goals || 0, assists: s.assists || 0, motm: s.motm || 0, cs, conceded: conc, win: feat ? Math.round(w / feat * 100) : 0 };
  });
  var me = rows.filter(function(r) {
    return r.num === p.num;
  })[0] || {};
  var maxOf = function(k) {
    return Math.max.apply(null, rows.map(function(r) {
      return r[k] || 0;
    }).concat([1]));
  };
  var counters = isGK ? [["Apps", "apps"], ["Starts", "started"], ["Clean sheets", "cs"], ["Conceded", "conceded"], ["MOTM", "motm"], ["Win %", "win"]] : [["Apps", "apps"], ["Starts", "started"], ["Goals", "goals"], ["Assists", "assists"], ["MOTM", "motm"], ["Win %", "win"]];
  var ach = counters.filter(function(c) {
    return ["apps", "started", "conceded"].indexOf(c[1]) === -1;
  });
  var best = null, bestPct = -1;
  ach.forEach(function(c) {
    var k = c[1], v = me[k] || 0, mx = k === "win" ? 100 : maxOf(k), pc = mx ? v / mx : 0;
    if (v > 0 && pc > bestPct) {
      bestPct = pc;
      best = k;
    }
  ...
  return h("div", { className: "m-counters" }, counters.map(function(c, i) {
    ...
    return h(
      "div",
      { className: "m-counter" + (isBest ? " is-standout" : ""), key: i },
      h(
        "svg",
        { viewBox: "0 0 64 64", className: "m-counter__svg" },
...
function PercentileWheel(props) {
  ...
  var rows = squad.map(function(s) {
    ...
    var w = lg.filter(function(m) {
      return m.res === "w";
    }).length;
    return { num: s.num, goals: s.goals || 0, assists: s.assists || 0, ga: (s.goals || 0) + (s.assists || 0), apps: s.apps || 0, started: s.started || 0, motm: s.motm || 0, cs, csrate: feat ? Math.round(cs / feat * 100) : 0, w };
  });
  var me = rows.filter(function(r) {
    return r.num === p.num;
  })[0] || { goals: 0, assists: 0, ga: 0, apps: 0, started: 0, motm: 0, cs: 0, csrate: 0, w: 0 };
  var sum = function(k) {
  ...
  var played = allRes.filter(function(r) {
    return r.kind !== "walkover";
  });
  var isUs = function(r) {
    return r.home.indexOf("Angels") > -1;
  };
  var teamMatches = played.length || 1;
  var teamCS = played.filter(function(r) {
    return (isUs(r) ? r.as : r.hs) === 0;
  }).length || 1;
  var teamWins = played.filter(function(r) {
    var us = isUs(r) ? r.hs : r.as, th = isUs(r) ? r.as : r.hs;
    if (r.kind === "penalty" && r.pens) {
      var up = isUs(r) ? r.pens.hs : r.pens.as, tp = isUs(r) ? r.pens.as : r.pens.hs;
      return up > tp;
    }
    return us > th;
  }).length || 1;
  ...
  var metrics = grp === "GK" ? [M.cs, M.csrate, M.win, M.motm, M.avail, M.starts] : grp === "DM" ? [M.cs, M.contrib, M.assists, M.motm, M.avail, M.starts] : grp === "DEF" ? [M.cs, M.goals, M.assists, M.motm, M.avail, M.starts] : [M.goals, M.assists, M.contrib, M.motm, M.avail, M.starts];
  ...
  function wedge(i, r) {
    ...
    return "M" + cx + " " + cy + " L" + s[0].toFixed(1) + " " + s[1].toFixed(1) + " A" + r + " " + r + " 0 0 1 " + e[0].toFixed(1) + " " + e[1].toFixed(1) + " Z";
  }
  function enter(i) {
    return function() {
      setHover(i);
  ...
  return h(
    ...
    h(
      "svg",
      { viewBox: "0 0 360 360", className: "m-wheel__svg" },
      h("defs", null, h("clipPath", { id: "m-wheelclip-" + p.num }, h("circle", { cx, cy, r: inR }))),
      [0.5, 0.75, 1].map(function(f, i) {
        return h("circle", { key: "r" + i, cx, cy, r: inR + (maxR - inR) * f, fill: "none", className: "m-wheel__ring" });
      }),
      metrics.map(function(m, i) {
        var v = m[1], r = inR + v / 100 * (maxR - inR);
        return h("path", { key: "w" + i, d: wedge(i, r), className: "m-wheel__wedge" + (hover === i ? " is-active" : ""), style: { opacity: (0.3 + 0.58 * (v / 100)).toFixed(2) }, onMouseEnter: enter(i), onMouseLeave: leave });
      }),
      metrics.map(function(m, i) {
        var a = (i * 60 - 90) * Math.PI / 180, e = pt(a, maxR);
        return h("line", { key: "l" + i, x1: cx, y1: cy, x2: e[0].toFixed(1), y2: e[1].toFixed(1), className: "m-wheel__spoke" });
      }),
      ...
      h("circle", { cx, cy, r: inR, fill: "none", className: "m-wheel__photoring" }),
      metrics.map(function(m, i) {
        var am = ((i + 0.5) * 60 - 90) * Math.PI / 180, lp = pt(am, maxR + 28), hit = pt(am, maxR + 22);
        return h(
          "g",
          { key: "t" + i, className: "m-wheel__lblg" + (hover === i ? " is-active" : ""), onMouseEnter: enter(i), onMouseLeave: leave },
    ...
    hm ? h(
      "div",
      { className: "m-wheel__tip", role: "tooltip" },
      h("strong", null, hm[2]),
      h("span", { className: "m-wheel__tip-d" }, hm[3])
    ) : null
  );
}
...
function saInjectFixtureSchema() {
  try {
    ...
    var events = ups.slice(0, 16).map(function(f) {
      ...
      ev.location = isHome ? { "@type": "Place", name: "The Reeves", address: { "@type": "PostalAddress", addressLocality: "Hanworth", addressRegion: "London", addressCountry: "GB" } } : { "@type": "Place", name: (away.replace(" FC", "") || "Opposition") + " ground (away)", address: { "@type": "PostalAddress", addressRegion: "London", addressCountry: "GB" } };
      return ev;
    }).filter(Boolean);
...
if (typeof window !== "undefined") {
  ...
  ["sa-roster-changed", "sa-media-changed"].forEach(function(evt) {
    window.addEventListener(evt, function() {
      setTimeout(saInjectFixtureSchema, 250);
    });
  });
