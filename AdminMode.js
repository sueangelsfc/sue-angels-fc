...
(function () {
  ...
  window.isAdmin = function () {
    return !!window.__sa_admin;
  };
  function emit() {
    window.dispatchEvent(new CustomEvent('sa-admin-changed'));
    renderPill();
  }
  ...
  if (!CLOUD) {
    ...
    try {
      ...
      if (url.searchParams.get('admin') === '1') {
        on = true;
        localStorage.setItem('sa-admin', '1');
      }
    ...
    window.setAdmin = function (next) {
      ...
      try {
        localStorage.setItem('sa-admin', next ? '1' : '0');
      } catch (e) {}
      emit();
    };
    ...
    if (document.readyState !== 'loading') renderPill();else window.addEventListener('DOMContentLoaded', renderPill);
  } else {
    // ───────────────────────────────────────────────── CLOUD (Supabase) ──
    ...
    async function syncAuth() {
      try {
        ...
        if (isAdmin !== window.__sa_admin) {
          window.__sa_admin = isAdmin;
          emit();
        }
        ...
        if (!window.__sa_auth_sub) {
          const sub = c.auth.onAuthStateChange((_e, session) => {
            ...
            if (adm !== window.__sa_admin) {
              window.__sa_admin = adm;
              emit();
            }
          });
    ...
    window.openAdmin = function () {
      ...
      setTimeout(() => {
        const f = overlay.querySelector('#sa-email');
        if (f) f.focus();
      }, 50);
    ...
    if (document.readyState !== 'loading') syncAuth();else window.addEventListener('DOMContentLoaded', syncAuth);
    ...
    try {
      ...
      if (u.searchParams.get('login') === '1') {
        if (document.readyState !== 'loading') window.openAdmin();else window.addEventListener('DOMContentLoaded', () => window.openAdmin());
      }
    } catch (e) {}
  }
  ...
  window.AdminSlot = function ({
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "admin-slot__head"
    }, /*#__PURE__*/React.createElement("span", {
      className: "admin-slot__chip"
    }, "ADMIN"), /*#__PURE__*/React.createElement("span", {
      className: "admin-slot__title"
    }, title)), /*#__PURE__*/React.createElement("div", {
      className: "admin-slot__body"
    }, children));
  };
  window.AdminBanner = function () {
    return null;
  };
  ...
  function mountScrollNav() {
    ...
    up.className = 'scroll-nav__btn';
    up.type = 'button';
    up.setAttribute('aria-label', 'Scroll to top');
    up.innerHTML = '&uarr;';
    const down = document.createElement('button');
    down.className = 'scroll-nav__btn';
    down.type = 'button';
