...
- **The home page's running order is the club's**, in Control panel → Home page. **Seventy bands across seven areas**, each movable, each able to be turned off, written to `home:layout`. It was one line of `home.mjs`, which made it a decision from July that the club could not revisit: in August the promotion to League Eight was news while an empty League Eight table led the page. `src/lib/home-layout.mjs` is the single definition and it is total, never throwing whatever the record holds, because a deploy now runs the generator and a record that threw would fail the club's own publish.
  - **The seventy, by area:** *Happening now* (11) · *On the pitch* (10) · *The figures* (15) · *The people* (11) · *The club* (6) · *Getting involved* (8) · *The archive* (9). `HOME_BANDS` in `src/lib/home-layout.mjs` is the list; the panel is handed it by the build.
  - **The live page is 24 bands: the twenty as shipped, less the four the club hid, plus the archive.** The other 41 are off until switched on. Not because they are unfinished: the club had already arranged its front page, and bands arriving switched on would rearrange it without anybody asking. **Show everything** turns the lot on in one press.
  ...
  - **A band with no data is a switch that lies**, so every candidate is checked against the records before it is written. Three were dropped for exactly this: goalkeeper saves (`keeperApps` is 0 across the squad), a video band (no match carries one), and ever-present players (nobody has played all 33). `PARTNERS` turned out to be a subset of `SPONSORS`, so that band would have duplicated Who backs the club.
