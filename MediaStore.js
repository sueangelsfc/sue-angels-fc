...
if (typeof window !== 'undefined' && window.dataStore && window.dataStore.gallery) {
  ...
  window.GalleryStore = {
    ...
    update: (id, patch) => {
      ...
      if (!cur) return;
      Promise.resolve(window.saveGalleryAlbum({
        ...cur,
        ...patch
    ...
    reorder: (id, dir) => {
      ...
      const as = a.sort || 0,
        bs = b.sort || 0;
      Promise.all([window.saveGalleryAlbum({
        ...a,
        sort: bs
      }), window.saveGalleryAlbum({
        ...b,
        sort: as
      })]).then(ping).catch(handleErr);
    }
...
window.useMediaStore = function (store, prefix) {
  ...
  React.useEffect(() => {
    const h = e => {
      if (!e.detail || e.detail.prefix === prefix) setItems(store.list());
    };
