...
- **League Eight is pinned under the home page hero while it has a match played** (`leagueNowBand` in `home.mjs`): the table, the round just played, the next round and the scorers. The club's saved layout has the table, results and fixtures bands off and only the panel can change that record, so the division the club plays in was nowhere on its own front page. It is the one band that does not wait for the layout, it empties itself before a ball is kicked, and the panel lists it as pinned. **The scorers are transcribed** from Full-Time's stat leaders page into `scorers` (goals only; cards need every page of that list), shown under the League Eight table on the league page, and `compareScorers()` in `stats.mjs` fails `npm run verify` if a club's scorers come to more goals than its results.
